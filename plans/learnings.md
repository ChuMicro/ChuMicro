...
The root cause turned out to be **two distinct bugs compounding**:

1. **CP autoreload trampling on host writes.** `_stage_to_flash` and `deploy_files` both did host-side drive operations (`.chu-probe` write, `boot_out.txt` read, `neuter_macos_metadata` sentinel plants, `.Spotlight-V100` rmtree) BEFORE sending `supervisor.runtime.autoreload = False` over the serial REPL. With autoreload ON (the default), CP's filesystem watcher fires a soft-reboot on each file change. Each soft-reboot re-enumerates USB-CDC. Multiple re-enumerations during prep left the board's USB-CDC stack in a degraded state, so when rsync started its first batch of writes the next `write()` landed in uninterruptible kernel I/O wait. Fix: `_disable_autoreload_before_drive_writes()` helper called as the FIRST thing after `_enter_raw_repl()`, before any drive operation. Symmetric `_restore_autoreload()` helper consolidates the disconnect-side restore. Regression test `test_flash_stage_disables_autoreload_before_any_drive_write` snapshots wire vs. filesystem event ordering.

2. **No `timeout=` on the rsync subprocess.** Once the kernel I/O wedged, rsync had no way out. SIGTERM/SIGKILL only fire when the process can be reaped — a child stuck in D-state is unkillable until the underlying USB connection is forcibly closed via board reset. Fix: every CIRCUITPY-touching subprocess in `chumicro_deploy.flash_drive` now passes `timeout=` (`RSYNC_TIMEOUT_SECONDS=90`, `SYNC_TIMEOUT_SECONDS=30`, `METADATA_HELPER_TIMEOUT_SECONDS=10`). On `subprocess.TimeoutExpired` the deploy raises `FlashDriveError` with the recovery procedure ("reboot the board"). Timeout enforcement is best-effort — if the parent's `waitpid` itself blocks, the timeout can't fire — but for the common "rsync got 95% through and the next `write()` hangs" pattern it converts a process wedge into a clean error.
