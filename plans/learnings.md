...
### `from __future__ import annotations` is invalid on MicroPython

Library code under `libraries/*/src/` and `support/test_harness/src/` must NOT include `from __future__ import annotations` — MicroPython (1.26.0 confirmed) has no `__future__` module, and the import fails before any annotations are evaluated.  All existing libraries (timing, runner, msgpack, compat) use PEP 604/585 syntax (`int | None`, `list[int]`) directly without the future import; both runtimes parse these as forward references via PEP 563-style behavior even without the explicit opt-in.  Surfaced when kvstore Slice 2 functional tests on real hardware failed with `ImportError: no module named '__future__'` even though host-side CPython tests passed.  Fix: strip the `from __future__ import annotations` line from every library src/ module; tests + scripts can keep it (CPython-only).  Decision 0021 already mandates the syntax; this is the implementation note that goes with it.
