...
**Switching to mbedtls is the obvious fix and matches every real CP board, but CP 10.1.4's source tree is internally inconsistent and the mbedtls build can't actually link.**  `lib/mbedtls/` is pinned at v2.28.3 (mbedtls 2.x source layout — `ssl_cli.c` + `ssl_srv.c` + `rsa_internal.c`), while `extmod/extmod.mk` lists v3.x file names (`ssl_client.c` + `ssl_server.c` + `bignum_core.c` + `rsa_alt_helpers.c` + `ssl_debug_helpers_generated.c` + `ssl_tls12_*.c`) that don't exist in v2.28.3.  The link step fails with "no such file or directory" on every missing v3.x source.  Two extra build knobs were also needed and discovered along the way: (a) `MBEDTLS_CONFIG_FILE` must be overridden to point at CP's own `lib/mbedtls_config/mbedtls_config.h` because CP didn't bring over MP's `extmod/mbedtls/mbedtls_config_common.h` that the default port config transitively includes, and (b) the rp2 port's Makefile demonstrates the correct override pattern.  Both stop mattering once the link fails, of course.

Two ways forward, neither attempted in this session:

1. **Update CP's mbedtls submodule to v3.x** — CP-side decision; affects every board's per-board build, not just the unix-port.  Probably the right answer upstream but out of scope for chumicro.
2. **Patch a CP-2.x-compatible source list into the unix-port build** — write a custom `MBEDTLS_SRC` list in `prepare_circuitpython.py` that uses only the files CP actually ships, instead of inheriting `extmod.mk`'s 3.x-shaped list.  Fragile and needs maintenance every time CP rebases on MP.

Until one of those happens: CP unix-port stays at "hashlib full, ssl absent".  Tests that need `import ssl` directly are CPython-only or MP-unix-only.  For comparison, MP unix-port has full `ssl` via mbedtls (its submodule and its file list match — because MP is the one that updated both).  The discrepancy is purely the CP-side mbedtls-version drift.
