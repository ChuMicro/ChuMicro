...
### Per-tick `bytearray(N)` allocations are invisible on host but fragment small-heap boards

On CPython the allocator pools 512-byte / 1024-byte short-lived `bytearray`s into PyMalloc arenas — `bytearray(512)` per-tick is free.  On MP/CP small-heap ports the alloc lands directly in the GC's `byteish` block tier; freeing it doesn't immediately reclaim the slot, and the next tick's alloc may take a different one.  Over hundreds of ticks the tier's free-block count drifts down — the on-device fragmentation tests at `libraries/{requests,http_server,websockets}/functional_tests/test_memory_fragmentation_on_device.py` measure this directly via `gc.mem_alloc()` histograms.

What this means for library code: any `bytearray(N)` in a per-tick or per-call hot path needs to move to construction-time pre-allocation (with `memoryview` views handed to consumers).  See the **Static recv buffer + memoryview window** pattern in `plans/patterns.md`.  The bug is silent in host pytests — the only signal that catches it is the on-device fragmentation suite running on Lolin S2 (Pi Pico W's heap is too small to expose tier-1024 resolution).  Found in 2026-05-03 audit of chumicro-requests / chumicro-http-server `_drive_recv` and chumicro-websockets `_HandshakeLineParser` after the original ResponseParser fragmentation fix exposed the wider pattern; the websockets `_session.py` already had the pre-alloc pattern (caught earlier on Pi Pico W when adding the websockets recv loop) — that turned out to be the right reference.
...
**The ssl module does not come along for the ride.**  CP ships an `ssl.py` shim (under `lib/micropython-lib/python-stdlib/ssl/`) that does `import tls` — `tls` is the C-level TLS module that gets compiled from `extmod/modtls_mbedtls.c` (mbedTLS-backed) or `extmod/modtls_axtls.c` (axtls-backed), depending on which build flag is set.  Choosing axtls compiles `modtls_axtls.c`, but `modtls_axtls.c` doesn't actually create the `tls` module — only `modtls_mbedtls.c` does.  Result: `import ssl` on the CP unix-port still fails with `ImportError: no module named 'tls'` even after the SSL+axtls enable.

**Switching to mbedtls is the obvious fix and matches every real CP board, but the CP unix-port build has three independent problems that all have to be solved together — and even if they were, the test recovery payoff turns out to be zero.**

Problem 1 — *CP source tree is internally inconsistent on mbedtls.*  `lib/mbedtls/` is pinned at v2.28.3 (mbedtls 2.x source layout — `ssl_cli.c` + `ssl_srv.c` + `rsa_internal.c`), while `extmod/extmod.mk` lists v3.x file names (`ssl_client.c` + `ssl_server.c` + `bignum_core.c` + `rsa_alt_helpers.c` + `ssl_debug_helpers_generated.c` + `ssl_tls12_*.c`) that don't exist in v2.28.3.  The link step fails with "no such file or directory" on every missing v3.x source.  Real CP boards work because they each roll their own mbedtls integration: `ports/raspberrypi/Makefile:585-657` defines its own `SRC_MBEDTLS :=` with v2.x file names; `ports/espressif/Makefile` uses esp-idf's bundled mbedtls instead of `lib/mbedtls/` at all.  Only the unix-port relies on `extmod/extmod.mk`'s mbedtls integration, and it broke when MP rebased past CP's pinned submodule.

Problem 2 — *Default `MBEDTLS_CONFIG_FILE` references a header CP doesn't ship.*  The default `ports/unix/mbedtls/mbedtls_config_port.h` does `#include "extmod/mbedtls/mbedtls_config_common.h"`.  CP keeps mbedtls config in `lib/mbedtls_config/` instead and didn't bring over the MP common header.  Fix: override `MBEDTLS_CONFIG_FILE` to point at CP's own `lib/mbedtls_config/mbedtls_config.h` (rp2's Makefile uses the same trick).

Problem 3 — *CP forked before MP added `extmod/modtls_mbedtls.c`.*  The `ssl.py` shim does `import tls`, where `tls` is the C module produced by either `extmod/modtls_axtls.c` or `extmod/modtls_mbedtls.c`.  But CP's `extmod/` directory has neither file — they're MP additions that post-date CP's fork.  Even if mbedtls compiled and linked cleanly, no `tls` module would be created and `import ssl` would still fail.  MP's `modtls_mbedtls.c` is 1013 lines and supports both 2.x and 3.x mbedtls (gated by `#if MBEDTLS_VERSION_NUMBER >= 0x03000000`), so it could be back-ported, but that's a meaningful patch to maintain.

**The kicker: fixing all three would recover zero additional cross-runtime tests.**  Audit of the remaining `_pytest`-only files (after this session's salvages) shows every one is blocked by something *other* than `import ssl`:

* Real loopback `socket.socket()` calls (no stdlib `socket` on CP unix either, even with mbedtls)
* The CPython-only `cryptography` PyPI library (used to mint test certs)
* `threading` (not on MP/CP unix)
* `monkeypatch.setattr("sys.platform", ...)` for the rp2-platform-reject test (`sys.platform` is read-only at the C level on the unix ports)

So the ssl-on-CP-unix path is purely "make CP unix-port more board-like for its own sake" — no test-recovery value.  Documenting and parking.

Until and unless someone takes on this whole stack of upstream-style work (and there's a reason to), CP unix-port stays at "hashlib full, ssl absent".  For comparison, MP unix-port has full `ssl` via mbedtls (its submodule, its file list, its `extmod/modtls_mbedtls.c`, and its config header all match — because MP is the one maintaining the integration).  The discrepancy is purely the CP-side fork-and-drift situation.
...
### Partial-mock tests hide live-state couplings — stub every I/O boundary the production code touches

A unit test that monkeypatches *some* I/O boundary functions but not *all* of them silently couples to whatever live state the un-mocked function reads. The smell: a test specifies "what would happen if these files changed" but its assertions only hold given the current state of unrelated files. Symptom: the test passes for months and then breaks when something completely unrelated changes — usually a version bump, package rename, or workspace restructure.

Concrete example: `scripts/tests/test_check_version.py::test_pre_release_floor_skips_bump_requirement` monkeypatched `check_version.changed_files` and `check_version.release_tags` but left `check_version.read_version` reading real on-disk `VERSION` files. The test hardcoded `workbench/repl` as its `0.0.0` example. When commit `236c229` bumped that VERSION to `0.0.1`, the test broke — even though no `check_version` logic had changed and no test code had been edited. Eight other tests in the same file had the same latent issue; refactoring all 14 to use a synthetic `_synthetic_versions` fixture that monkeypatches `read_version` per-test eliminated the coupling category. When in doubt, stub it. The exception is *integrity tests* (e.g., "every publishable package must have a `VERSION` file") which deliberately assert workspace structure — those should be docstring-tagged as such so a future reader doesn't refactor them.
...
**Update 2026-04-26 — file count *is* the dominant flash cost on Pi Pico W.**  The 800 KB CIRCUITPY drive on Pi Pico W (FAT12, 4 KB clusters) means every `.mpy` file pays ≥ 4 KB on disk regardless of how small it actually is.  At 51 source files in the workspace, that's ~204 KB of pure cluster overhead before any content.  The earlier "not worth it: inlining modules" verdict was written assuming RAM was the bottleneck — when flash is the bottleneck, file count flips from neutral to load-bearing.  Two cuts that reflect this: (a) `testing.py` declares `__chumicro_runtimes__ = ("cpython",)`, so the marker filter (Decisions 0037 + 0044) drops it from every bundle and every host-side deploy path (~24 KB across 6 libraries that have it); (b) `chumicro-mqtt` consolidated 8 source files (`_packets`, `_encoder`, `_decoder`, `_errors`, `_state`, `client`, `__init__`, `testing`) → 4 (`_wire`, `client`, `__init__`, `testing`), with `testing` filtered out by the same marker = 3 device files (saves ~16-20 KB FAT cluster cost on MQTT alone).  Traceback clarity stays acceptable because the splits were across one logical concern (wire format) — keeping `client` separate preserves the orchestration/wire boundary.
