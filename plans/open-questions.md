...
### A release identifies the same snapshot three ways

**Surfaced 2026-07-18** during the first real end-to-end publish
(Decision 0111). One logical release stamps three uncorrelated
identifiers: PyPI semver per package (`chumicro-wifi 0.8.0`), the device
bundle's date tag (`20260718`), and the libraries channel's timestamp
tag (`20260718.153658`). Nothing maps one to another, so a bug reported
against a PyPI version cannot be traced to a bundle or libraries
snapshot.

**Open:** should a release carry one correlating key (a manifest that
records, per snapshot, which PyPI versions and which bundle/libraries
tags it comprises), or is per-channel independence acceptable because
each channel is self-consistent? Decisions 0078 and 0101 chose
"channel is a repo, latest tag wins, no version solver" for the device
and host channels; this asks only for a lookup, not a solver.

Related: Decision 0111, Decision 0101, Decision 0078.

### The device bundle hard-codes one mpy ABI per runtime

**Surfaced 2026-07-18** (Decision 0111). The bundle repo lays out
exactly one MicroPython format folder (`mpy6/`) and one CircuitPython
folder (`circuitpython-10.x-mpy/`), keyed by today's magic bytes
(`bundle_layout.py`). When MicroPython ships `.mpy` v7 or CircuitPython
moves to an incompatible 11.x format, boards on the old runtime need the
old bytecode and boards on the new one need the new, from the same
channel.

**Open:** what selects the right mpy variant per board as ABIs
proliferate? Parallel format folders plus a circup/mip resolver that
reads the board's runtime version, versus a per-runtime bundle repo,
versus dropping old formats on a support-window schedule. Not pressing
while exactly two live formats exist; becomes real at the next ABI bump.

Related: Decision 0111, Decision 0101, Decision 0024.
