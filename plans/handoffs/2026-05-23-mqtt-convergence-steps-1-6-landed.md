...
5. **Self-heal cost sandboxing** (deferred from step 6 polish).  `_attempt_self_heal` runs inside `handle()` (a runner tick), so its synchronous `socket_factory()` call — DNS + TCP + TLS handshake — stalls every other registered service for seconds.  The reference impl's pattern is **reconnect-in-user-space, not in the library**: `loop()` raises `MQTTSocketError` on a dead connection; the application catches it and calls `start_connect()` again from `main`, OUTSIDE the loop.  Blocking is allowed at startup / between loop runs; loop itself never blocks.  Two design options:
    - **(a) Match the reference**: remove `_attempt_self_heal` entirely, surface the failure to the application, and document that callers handle reconnect.  Simplest; loses the chumicro-side convenience of "self-heal just works."
    - **(b) Keep self-heal but break it across ticks** via a state machine (CLOSING → AWAITING_FACTORY → CONNECTING → AWAITING_CONNACK), each phase tick-bounded.  More complex; preserves the convenience.
