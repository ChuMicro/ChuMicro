...
5. **Connect-path cost sandboxing** (deferred from step 6 polish, broader than originally framed).  The original framing was "self-heal blocks the runner during DNS + TCP + TLS."  User pointed out 2026-05-23 that the initial `connect()` calls the same `_socket_factory()` synchronously and does the same network work.  The difference is contract-based, not implementation-based: initial `connect()` is user-controlled timing (typically before `runner.tick()` ever runs, so "blocks boot" not "blocks tick"), while `_attempt_self_heal` runs INSIDE `handle()` which is a runner tick — every other registered service stalls.  The clean fix breaks BOTH paths via a state machine (CLOSING → AWAITING_FACTORY → CONNECTING → AWAITING_CONNACK) so calling `connect()` from anywhere (including inside a runner-tick handler) is safe.  Tracked in next-up — needs an ADR before code lands.
