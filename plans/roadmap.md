...
- ✅ live end-to-end VS Code Testing-panel verification pass against hardware (closed 2026-04-25 — `git log --since=2026-04-25 --until=2026-04-26`)
...
- ✅ **Phase 4a** — `workbench/workspace-runtime/` (`chumicro-workspace`) — host CLI (17+ commands), three-zone `devices.yml` round-trip, multi-thing-on-one-device deploys + `switch`, firmware URL derivation (CP S3 listing + MP curated map), import-graph resolver, on-device boot module shipped as payload.
