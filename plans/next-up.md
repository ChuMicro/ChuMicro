...
- [x] **`chumicro-deploy` interactive recovery layer** — new `InteractiveDeployer` + `classify_deploy_failure` + `RecoveryPlan` + `DeployFailureKind` exported alongside the deterministic `Deployer`.  Catches `CircuitpythonTransportError` / `MicropythonTransportError`, classifies into nine kinds (port unavailable, raw REPL unresponsive, CIRCUITPY drive missing, flash copy failed, bootstrap exec failed, insufficient memory, traceback returned, configuration error, unknown), prints a canned `RecoveryPlan` with ordered fix-steps, and runs a retry-loop with injectable `prompt` / `output` callables.  Fixes the spurious `WARNING: Failed to restore board state on disconnect` noise users saw when going into bootloader mode by marking `_reset_pending` on the CP transport so disconnect skips the restore dance after an intentional reset.  New `workbench/deploy/functional_tests/demo_recovery_hand_holding.py` walks every configured `devices.yml` board through the scenario set (happy-path baseline, traceback-on-board, unplug, eject-CIRCUITPY, bootloader-reset-silent).  59 host-side tests + coverage gate at 94.25 %.
