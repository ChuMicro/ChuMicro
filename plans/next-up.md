...
- [ ] **Phase 4a: `chumicro-workspace-runtime` remaining slices** — Slices 0–4 landed.  Slice 0 (deploy-time config-merge core).  Slice 1 (`WithRuntimeConfig` FileSource decorator).  Slice 2 (CLI dispatch — 17 commands).  Slice 3 (three-zone `devices.yml` reader/writer per Decision 0029 §9 via `ruamel.yaml`).  Slice 4 (onboarding flows: `BoardState` (REPL_REACHABLE / UF2_BOOTLOADER / NO_PROBE_RESPONSE / SERIAL_UNREACHABLE) + `OnboardingDiagnosis` + `detect_board_state` host detective + `find_uf2_drive` scanner; `add-device` integrates so probe failures surface human-readable next steps pointing at `install-firmware` / `discover` / etc.).  182 host tests at 97 % cov.  Remaining: Slice 5 (firmware URL derivation — CP S3 listing + MP scrape-and-cache), Slice 6 (import-graph resolver via AST walk from thing entrypoint), Slice 7 (device-side `workspace_runtime` boot module).
