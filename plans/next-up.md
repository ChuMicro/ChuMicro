...
- [ ] **Extend `check-version` + `check-api` to cover workbench packages** before bumping any `workbench/*/VERSION` off `0.0.0`.  Today both gates only walk `libraries/*/VERSION` (see [scripts/check_version.py:39](scripts/check_version.py:39)), so a workbench `src/` change without a VERSION bump won't fail preflight, and removed-symbol regressions in `chumicro-deploy` / `chumicro-repl` won't fail check-api against the previous tag.  Workflow-side release machinery already handles workbench (release.yml + promote.yml as of 2026-04-25), but the pre-merge gates haven't caught up.  Order of operations: extend the gates first, *then* do the first non-zero workbench bump so the gates are exercised end-to-end before they matter.
