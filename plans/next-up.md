...
- [ ] **Extend `release.yml` + `promote.yml` to cover workbench packages.**  Both workflows currently watch `libraries/*/VERSION` only and iterate `libraries/*/VERSION` files when staging the release set.  Workbench packages (`chumicro-deploy`, `chumicro-repl`, future `chumicro-workspace-runtime` / `chumicro-workspace-template`) will not trigger PyPI releases when their VERSION bumps.  Both deploy and repl sit at 0.0.0 today so it has not surfaced, but the first version bump for either will hit this.  Sketch: change the path filter to `{libraries,workbench}/*/VERSION`, change the per-VERSION loop to scan both directories, skip the bundle-staging + `.mpy`-compile stages for workbench entries (Decision 0032 rule 3 — workbench is PyPI-only).  Verify against an experimental release of `chumicro-deploy` before applying it to repl.
