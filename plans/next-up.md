...
- [ ] **ChuMicro project-workspace workstream** (Decision 0029). Multi-library effort: `chumicro-deploy` transport extraction, `chumicro-workspace-runtime` (host CLI + on-device boot module), `chumicro-repl` (CP/MP-aware serial TUI), `chumicro-wifi` (non-blocking connection manager), `chumicro-mqtt` (refactor prior hand-rolled client into a runner-shaped service), and a companion `chumicro-workspace-template` repo. Supersedes the terse `chumicro-deploy` entry previously carried here; scope + alternatives + rejections recorded in Decision 0029.
