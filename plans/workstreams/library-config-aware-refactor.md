...
Status: **all phases shipped + bench-validated CP-side 2026-05-08 — Phases 0 + 1 + 2 (mqtt / ntp / requests / websockets / http_server) + 3 (`deploy-example` CLI per Decision [0059](../decisions/0059-deploy-example-front-door.md)).  CP-side bench session ran `timing/circuitpython_blink` + `ntp/circuitpython_ntp_query` + `requests/circuitpython_periodic_get` against both Pi Pico W CP and Lolin S2 CP; all six deploys succeeded end-to-end with WIFI_OK, NTP_OK (matching real wall-clock), and HTTP 200 responses observed via the deploy command's serial capture.  Three follow-ups uncovered (recorded under "Follow-ups from the bench session" below): (1) mqtt example's PEP 448 `**unpack` rejected by CircuitPython parser — **shipped 2026-05-08** (rewritten to temp-variable merge, bench-validated on Pi Pico W CP); (2) MP `deploy-example` of infinite-loop examples times out via mpremote raw-exec — **investigated 2026-05-08, scoped, ready to implement** (gap is workspace-wide; concrete 3-4 hr fix sketched: add MP transport `follow="soft_reboot"` kwarg mirroring CP's flash-mode soft-reboot pattern); (3) CIRCUITPY UID stale-warning over-states the situation — **shipped 2026-05-08** (Variant A: silence on success).  Only item 2 implementation remains; none block the workstream's `from_config` factory acceptance.**
...
**What we know.**  `python scripts/run.py deploy-example timing micropython_blink --device pi-pico-w-micropython-board --non-interactive` exits 4 with `Device deploy-execute failed: timeout waiting for first EOF reception` (classified `bootstrap_exec_failed`).  `timing/examples/micropython_blink.py` is `while True: heartbeat.poll(...) ...` — never exits and never `print`s.  The error fires after `_EXECUTE_IDLE_TIMEOUT` = 300 s (`workbench/deploy/src/chumicro_deploy/micropython_transport.py:71`) of consecutive serial silence.

**Investigation findings (2026-05-08).**  Done over a focused session — bench-tested all primitives on Pi Pico W MP at `/dev/cu.usbmodem112401` and read the relevant code paths.

*The gap IS workspace-wide, not `deploy-example`-only.*  `chumicro-workspace deploy` and `deploy-example` both route through `Deployer.deploy_diff → transport.deploy_files` (`workbench/deploy/src/chumicro_deploy/deployer.py:233`).  `MicropythonTransport.deploy_files` always ends with `self._serial.exec_raw(script, timeout=_EXECUTE_IDLE_TIMEOUT)` at line 725 — both `mode="copy"` (flash equivalent) and `mode="mount"` (RAM equivalent) execute the entrypoint synchronously via raw-REPL and wait for the EOF (`\x04`) marker that an infinite loop never emits.  Any MP project with `while True: ...` in its entrypoint hits this regardless of which CLI front-door is used.

*MP has the same primitives as CP's flash-mode soft-reboot pattern.*  Bench-confirmed:

* **Ctrl-D from friendly REPL → soft-reboot.**  Output: `MPY: soft reboot` (analog of CP's `soft reboot` marker).  MP then runs `boot.py` (if present) and auto-runs `main.py`.  No mpremote subcommand needed — direct serial write of `\x04` from the persistent serial connection works.
* **Ctrl-C interrupts a busy `while True` cleanly.**  Drops to friendly REPL with `KeyboardInterrupt` traceback + `>>>` prompt.  No special handling needed.
* **`mpremote fs cp` reaches a busy board** because mpremote internally sends Ctrl-C+Ctrl-A to break into raw REPL before doing the filesystem op.  The existing `_run_mpremote(["fs", "cp", "-r", ...])` step in `deploy_files` already works on a busy device.
* **Friendly REPL prompt (`>>>`)** is the analog of CP's `Code done running.` end-of-output marker.  After a non-`while True` `main.py` returns, MP prints `>>>`.  After an exception, MP prints traceback + `>>>`.  After `while True`, no prompt ever appears.

*CP transport already has the right structural shape we need to mirror.*  `CircuitpythonTransport.deploy_files` (`workbench/deploy/src/chumicro_deploy/circuitpython_transport.py:1453-1456`) sends Ctrl-B (exit raw REPL) + Ctrl-D (soft-reboot from friendly REPL) — *no `exec_raw`* — then calls `_read_code_py_output()` which reads up to `self.timeout` seconds, syncs on `soft reboot` start marker + `Code done running.` end marker, and returns partial output on read-timeout.  Docstring notes explicitly: *"For infinite-loop entrypoints the `Code done running.` marker never appears and the read times out at `self.timeout` seconds — callers receive the accumulated output up to that point."*  The MP analog is the same shape with different markers.

*What we deliberately did NOT investigate.*  The user-perceived UX of "deploy succeeded but stayed running for `self.timeout` then returned with partial output" — that's a doc/help-text question for the implementation session, not a primitive question.  The single-process write-then-read race semantics (mpremote's auto-reset on exit, etc.) — irrelevant because the fix uses the persistent serial connection that's already open after `_run_mpremote(["fs", "cp", ...])` completes, mirroring the CP pattern.

**Proposed fix shape (now concrete).**

1. **Add `_read_main_py_output(self) -> str` to `MicropythonTransport`** mirroring `CircuitpythonTransport._read_code_py_output`.  Sync on `MPY: soft reboot` start marker; treat `>>>` (friendly REPL prompt) as the end-of-output marker; bound the read at `self.timeout` seconds; return whatever accumulated.  Reuse the same `_extract_code_output`-style boundary trimming.

2. **Add a `follow` kwarg to `MicropythonTransport.deploy_files`** (default `"exec"` — current behaviour, preserved for the test harness which deploys `test_*.py` files that DO return).  New value `"soft_reboot"`: after the `mpremote fs cp` step, send `\x02` (Ctrl-B exit raw REPL), `\x04` (Ctrl-D soft-reboot), then `_read_main_py_output()`, then re-enter raw REPL via the existing `_ensure_serial`/`_enter_raw_repl` machinery.  Constraint: requires `entrypoint == "/main.py"` (MP's auto-run convention); raise `MicropythonTransportError` early on mismatch.  CP's `mode="ram"` doesn't have this issue because it has its own `_deploy_files_ram` branch — same shape applies here for the `follow="soft_reboot"` branch.

3. **Wire the new mode through `Deployer.deploy_diff`** as an opt-in.  `chumicro-workspace deploy` and `deploy-example` for MP devices pass `follow="soft_reboot"`; the test-harness pytest-device path keeps `follow="exec"`.  Decision needed in implementation session: does CP's transport gain a parallel `follow` kwarg (currently it just always does soft-reboot)?  Probably yes for symmetry — a CP-side `follow="exec"` could collapse the bootstrap path into a single shape across both runtimes.  Defer that question until the MP side lands.

4. **Update `_ENTRYPOINT_BY_RUNTIME` callers** so `chumicro-workspace deploy` of an MP project defaults the entrypoint to `/main.py` (currently `WorkspaceDeploySource` defaults to `/code.py`).  `deploy-example` already does this correctly via `_ENTRYPOINT_BY_RUNTIME["micropython"] = "/main.py"`.
...
* ~30 min: add `_read_main_py_output` helper + tests (mirror existing CP tests).
* ~45 min: add `follow` kwarg to `deploy_files` + tests for both modes.
* ~30 min: ADR (if we go that route).
* ~45 min: thread `follow` through `Deployer.deploy_diff` + workspace deploy entrypoint default + tests.
* ~30 min: doc updates (help text, troubleshooting).
* ~30 min: bench-validate on Pi Pico W MP + Lolin S2 MP — deploy `timing/micropython_blink` end-to-end and confirm partial-output capture works.

**Sub-question resolved.**  `mpremote reset` is a hard reset (Ctrl-C interrupt → raw-REPL `machine.reset()`) — heavier than needed.  `mpremote soft-reset` exists but its output behaviour is opaque from the host side once mpremote disconnects.  The right path is direct serial Ctrl-B + Ctrl-D from the persistent connection, exactly mirroring CP's transport.  No new mpremote dependency required.
...
- **Status:** 1 + 3 shipped 2026-05-08; 2's investigation done same day, ready to implement.
- **Pickup recommendation for follow-up 2:** the fix shape is now concrete (see follow-up 2's "Proposed fix shape" — five-step plan, 3-4 hours).  Implementation session can start by drafting the ADR (or skipping it if we decide the kwarg is internal-enough), then move straight to the MP transport changes.  No further investigation needed unless the implementation surfaces a new question.
