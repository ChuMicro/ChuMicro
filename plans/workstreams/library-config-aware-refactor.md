...
Status: **all phases shipped host-side — Phases 0 + 1 + 2 (mqtt / ntp / requests / websockets / http_server) + 3 (`deploy-example` CLI per Decision [0059](../decisions/0059-deploy-example-front-door.md)).  Five refactored Phase 2 examples plus the Phase 3 deploy-example pipeline await a single bench session against the four-board canonical matrix to close the workstream end-to-end (see "Recommended pairing with hardware validation" below).**
