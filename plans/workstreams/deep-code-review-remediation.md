...
**Shipped 2026-06-13** (http_server 0.14.1, msgpack 0.1.31). MSG-1 turned out to be CPython-only: MicroPython / CircuitPython `struct.unpack_from` already raises `ValueError` on a short buffer and exposes no `struct.error`, so the catch tuple is built with `hasattr(struct, "error")`.
...
**Shipped 2026-06-13** (ntp 0.8.8, sockets 0.10.2, wifi 0.4.7, requests 0.13.7, http_server 0.14.2, websockets 0.19.8). SOCK-1 / WIFI-1 / REQ-1 resolved by honest documentation, no behavior change, per the user guidance below; NTP-1 is a real errno fix; WS-1 dropped a per-feed iterator allocation. WS-1's allocation win still wants a Pico W bench (pending).
...
**Shipped 2026-06-13** (logging 0.3.0, mqtt 0.16.7, websockets 0.19.9, requests 0.13.8, kvstore 0.2.8). LOG-1 and MQTT-1 are correctness/API fixes; WS-2 and MQTT-3 are structurally-evident allocation reductions (behavior-preserving, unit-tested) whose magnitude still wants a Pico W bench; CFG-1 is documented, not memoized; MQTT-2 and MQTT-3's per-tick retransmit cap folded into the cooperative-tick-convergence workstream (the recv-N-vs-send-1 rate redesign and a retransmit-timing change both need a real-board bake there).
...
**Shipped 2026-06-13** (sockets 0.11.0, wifi 0.4.8, deploy 0.32.2). SOCK-3 / SOCK-4 / WIFI-2 / DEP-2 are fully unit-verified; SOCK-2's MP TLS handshake change is sound by construction (raw-socket `setblocking`, the same call the listener path uses) but its end-to-end validation needs a Pico W TLS connector bake (pending).
...
**DEP-1 + DEP-3 shipped 2026-06-13 (deploy 0.32.3); DEP-4 shipped 2026-07-03 (deploy 0.36.0), with a premise correction.** A full alignment read of both transports showed the review's "~3.4k duplicated lines across both files" was misdiagnosed: CP does its filesystem ops host-side (mounted CIRCUITPY + rsync/pathlib) while MP does them device-side (raw-REPL scripts), so most of the repetition was *MP-internal* — the umount guard ×8, the ensure-serial→exec_raw→classify shape ×5, a byte-identical `_rm` rmtree in two scripts, one `_walk` skeleton in both list scripts.  Shipped: `_drop_active_mount()` + `_exec_or_classify()` (MP), scripts + marker + parser consolidated into `_device_scripts.py` (generated sources verified byte-identical pre/post), and the genuinely cross-file pieces hoisted to `protocol.py` — shared `DeviceTransportError` / `MidDeployDisconnected` bases (recovery's catch + classifier fast-path now key on the bases), `TimeSource` (was an MP→CP import), `write_files_to_staging`.  The classifier's message prefixes are load-bearing and preserved byte-for-byte.  Mode early-outs left alone: one-liners over genuinely different mode vocabularies ("flash" vs "copy") — a helper would add indirection, not remove a failure mode.  Validated on hardware: all 16 workbench deploy functional tests (the suite's first run since 2026-05-19 — it had rotted; see the next-up device-matrix bullet) + `sweep-devices` demo and timing-functional lanes on three of the four bench boards (s2-cp dropped off USB mid-day, awaiting physical replug).
...
**Shipped 2026-06-13** (sockets 0.11.1, ntp 0.8.9, kvstore 0.2.9, http_server 0.14.3, mqtt 0.16.8, websockets 0.19.10, timing 0.5.1, deploy 0.32.4). MQTT-L2 was verified clean (no change). TIME-L1 (Heartbeat now raises on `period_ms >= TICKS_HALFPERIOD`) and DEP-L2 (guarded `int(Content-Length)`) carry tests; the rest are prose / comment corrections.
...
### Phase 9 — Ecosystem decision (CLOSED)

ECO-1 reported the AGENTS.md-referenced project skills missing from `.github/skills/`. As of 2026-06-13 they are restored and committed (task-checkpoint, git-commit, new-library, new-decision, audit-code/-comments/-docs/-embedded/-library/-skill, regen-comments, `_shared/` all present and tracked), and the downloaded marketplace skills coexist alongside them. Confirmed 2026-06-13: every skill referenced from AGENTS.md / `patterns.md` / `next-up.md` resolves, so no dangling reference remains.
...
- 2026-06-13 — Phase 7 (partial) shipped (deploy 0.32.3). DEP-1: `_run_mpremote` passes `timeout=_MPREMOTE_SUBPROCESS_TIMEOUT` (120 s) and converts `subprocess.TimeoutExpired` into a classified `MicropythonTransportError` with replug / reset-board guidance, so a wedged board can't hang the deploy forever. DEP-3: `_verify_drive_for_board` refuses (rather than returning an unverified drive) when the serial probe gives no identity and more than one CIRCUITPY* volume is mounted, closing the wrong-board `rsync --delete` window on multi-board hosts (single-board hosts unchanged). DEP-4 (the ~3.4k-line CP/MP transport dedup) deferred to its own bake-validated pass. 4 new tests; full preflight green.
- 2026-06-13 — Phase 6 shipped. SOCK-4 (sockets 0.11.0): `tcp_client_connector` / `tls_client_connector` added to `__all__` + the module Public-API docstring (minor bump — they were defined but unexported). SOCK-3: `FakeUDPSocket.recvfrom_into` now raises `OSError(EAGAIN)` on an empty queue, matching real CP/MP non-blocking UDP (a genuine zero-length datagram still returns 0); NTP unaffected (it handles EAGAIN), one fake-behaviour test updated and a zero-length-datagram test added. WIFI-2 (wifi 0.4.8): `WifiConfig.connect_timeout_ms` documented CP-only. DEP-2 (deploy 0.32.2): `_decode_exec_result` / `execute` guard the mpremote result on `len(result)`, accepting a 1- or 3-tuple instead of raising. SOCK-2 (sockets 0.11.0): the MP TLS connector flips the raw socket blocking for the inline handshake then back, so it can't return mid-flight; real-board TLS connector bake pending. 6 new tests; full preflight green.
- 2026-06-13 — Phase 5 shipped. LOG-1 (logging 0.3.0): deferred `%`-args past the level gate. MQTT-1 (mqtt 0.16.7): `_enqueue_user_tx` is variadic and atomic, so a QoS-0 callback's packet+marker are one capacity-checked unit. WS-2 (websockets 0.19.9, requests 0.13.8): send loops slice a cached memoryview instead of copying / rebuilding the base view per iteration (http_server already used the cached-view pattern). MQTT-3 (mqtt 0.16.7): the DUP-flagged retransmit is built once on `InFlightPublish` and reused. CFG-1 (kvstore 0.2.8): `bytes_used` documents its per-read full-encode cost. MQTT-2 and MQTT-3's per-tick cap folded into the cooperative-tick-convergence workstream. 11 new tests; WS-2 / MQTT-3 allocation wins are structurally evident, magnitude bench-pending. Full preflight green.
- 2026-06-13 — Phase 4 shipped. NTP-1 (ntp 0.8.8): recv accepts a runtime-built EAGAIN/EWOULDBLOCK set, fixing the macOS-CPython permanent-fail. SOCK-1 (sockets 0.10.2): both connector docstrings document the synchronous `getaddrinfo` in `awaiting_dns`. WIFI-1 (wifi 0.4.7): the `WifiService` docstring warns a CP connect stalls co-scheduled services up to `connect_timeout_ms`. REQ-1 (requests 0.13.7, http_server 0.14.2): corrected the "bounds tick latency" overclaim on `recv_budget_per_tick` — a budget above the 512 B scratch drains in up to `budget // 512` iterations per tick (a latency/throughput tradeoff), documented not clamped so the knob stays the user's lever. WS-1 (websockets 0.19.8): the masked-inbound unmask is hand-indexed instead of `range(take)`, dropping a per-feed iterator allocation; behavior-preserving (320 ws tests green), allocation win wants a Pico W bench. Full preflight green.
- 2026-06-13 — Phase 3 shipped. HTTP-1 (http_server 0.14.1): `encode_response` validates the reason phrase and every response header name/value for CR/LF/NUL via a server-local `_reject_control_chars`, raising `ServerProtocolError`, closing handler-reflected response splitting. HTTP-3: `_try_parse_request_line` rejects before slicing when `crlf_index > max_request_line_bytes`, so the 414 cap is exact rather than soft by up to one recv chunk. MSG-1 (msgpack 0.1.31): `unpackb` catches a truncated multi-byte header (CPython `struct.error` / `IndexError`) and re-raises `ValueError(_MALFORMED)`; the catch tuple is runtime-built because MP/CP raise `ValueError` directly and expose no `struct.error`. 13 new tests; full preflight green including MP/CP unix-port. Real-board bake (Pico W) pending.
- 2026-06-13 — Phase 8 shipped (sockets 0.11.1, ntp 0.8.9, kvstore 0.2.9, http_server 0.14.3, mqtt 0.16.8, websockets 0.19.10, timing 0.5.1, deploy 0.32.4). The LOW comment/doc cluster: SOCK-L1 (fake's `enqueue_eagain_for_*` method names), SOCK-L2 (UnsupportedSSLConfigError's second firing site), NTP-L1 (3-arg `sendto` contract), CFG-L1 (two non-atomic slice-assigns + the CRC backstop), CFG-L2 (softened the NVS claim, naming `unpackb` framing as the backstop), HTTP-L1 ("realloates" -> in-place-memmove compaction), MQTT-L1 (`on_oversized` topic can be None). Code-touching: WS-L1 (`unpack_from` for the close code), WS-L2 (`_next_auto_ping_ticks` initialised in `_init_session_state`, read directly), TIME-L1 (Heartbeat raises on `period_ms >= TICKS_HALFPERIOD`), DEP-L1 (`Path(...).name` on the firmware filename), DEP-L2 (guarded `int(Content-Length)`). MQTT-L2 verified clean. 4 new tests; full preflight green.
- 2026-06-13 — Phase 9 closed. ECO-1 confirmed resolved: every skill referenced from AGENTS.md / `patterns.md` / `next-up.md` resolves under `.github/skills/` (restored + tracked earlier this session); no dangling reference remains.
