...
The repo has four publishable libraries: `compat/`, `msgpack/`, `runner/`, and `timing/`. All build and pass preflight. Release automation is in place for tagging and GitHub Releases; PyPI publishing is scaffolded but disabled. The branching model (Decision 0019) and API breakage detection (Decision 0020) are defined and wired into CI. Stable releases (`main`) push to `ChuMicro/chumicro-bundle`; experimental releases (`develop`) push to `ChuMicro/chumicro-bundle-experimental` (Decision 0018). Both repos use the same directory names (no `_experimental` suffix) — channel separation is handled by the repo itself. `scripts/bundle.py` stages .py + .mpy + `package.json`; the `--experimental` flag points `package.json` URLs to the experimental repo. The `release.yml` bundle job produces circup-compatible zips with correct naming derived from the repo name (`{bundle_id}-{platform}-{tag}.zip`) and internal structure (`{basename}/lib/`). Only CP 10.x mpy bytecode is produced. Bundle repo policy: no examples, no per-library READMEs, no workflows — all automation lives in the source repo. Remaining items: create `develop` branch on GitHub, set as default, configure branch protection, enable PyPI publishing when ready, create `chumicro-bundle-experimental` repo, add `BUNDLE_TOKEN` secret.
