...
- **Release (`.github/workflows/release.yml`):** triggers on VERSION changes pushed to `develop` or `main`. Detects changed libraries, builds distributions, creates per-library git tags (with `-experimental` suffix for develop), and creates GitHub Releases (marked pre-release for develop). Both channels publish to the bundle repo — develop uses `_experimental` package directories, main uses base names. Supports `workflow_dispatch` with dry-run option. PyPI publishing is scaffolded but disabled until trusted publisher is configured.
...
The repo has four publishable libraries: `compat/`, `msgpack/`, `runner/`, and `timing/`. All build and pass preflight. Release automation is in place for tagging and GitHub Releases; PyPI publishing is scaffolded but disabled. The branching model (Decision 0019) and API breakage detection (Decision 0020) are defined and wired into CI. Both `develop` (experimental) and `main` (stable) publish to `ChuMicro/chumicro-bundle` (Decision 0018): experimental releases use `_experimental` suffixed package directories and `-experimental` tags; stable releases use base names. `scripts/bundle.py` stages .py + .mpy + `package.json` with `--experimental` flag support. Users differentiate by package name (e.g., `chumicro-timing-experimental` vs `chumicro-timing`); on-device import paths are the same regardless of channel. The `release.yml` bundle job produces circup-compatible zips with correct naming (`chumicro-bundle-{platform}-{tag}.zip`) and internal structure (`{basename}/lib/`), including per-platform mpy zips for `9.x-mpy` and `10.x-mpy`. Bundle repo policy: no examples, no per-library READMEs, no workflows — all automation lives in the source repo (Decision 0018). Remaining items: create the `develop` branch on GitHub, set it as default, configure branch protection, enable PyPI publishing when ready, add `BUNDLE_TOKEN` secret.
