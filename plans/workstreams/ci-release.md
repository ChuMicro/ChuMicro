...
- **CI (`.github/workflows/ci.yml`):** triggers on push to `main` and on all PRs. Jobs: `lint`, `test` (3.11/3.12/3.13 matrix), `verify-examples`, `build` (with artifact upload), plus PR-only: `version-check`, `api-check`, `docs-build`, MicroPython compatibility, CircuitPython compatibility.
...
- MicroPython cross-runtime unit tests (promoted from tier 2)
- CircuitPython cross-runtime unit tests (promoted from tier 2)
