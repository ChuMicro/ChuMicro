...
- **Release (`.github/workflows/release.yml`):** triggers on VERSION changes pushed to `develop` or `main`. Detects changed libraries, builds distributions, publishes to PyPI via trusted publishing (OIDC, environment "pypi"), creates per-library git tags (with `-experimental` suffix for develop), and creates GitHub Releases (marked pre-release for develop). Both channels publish to their respective bundle repos. Supports `workflow_dispatch` with dry-run option and library filter.
- **Docs deployment (`.github/workflows/docs-deploy.yml`):** triggers on push to `develop` and `main`. Deploys per-library docs to GitHub Pages via mike with `--deploy-prefix <lib>`. Push to `main` deploys `<version> stable`; push to `develop` deploys `dev experimental`. Also generates and deploys the root landing page.
