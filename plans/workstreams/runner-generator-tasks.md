...
Status: **shipped** (2026-05-25). Implements [Decision 0087](../decisions/0087-generators-for-sequential-io.md).
...
- 2026-05-25: Phase 4 (CHU lint rule banning `async def` / `await` / `import asyncio`) dropped. Rationale: zero asyncio code anywhere in the repo today, no current drift to catch, and a PR introducing async would be obvious at review. Adding ~270 LOC of rule + ~150 LOC of tests + ~100 LOC of deploy-staging hardening as enforcement for a wall no one is walking toward fails the AGENTS.md "no infrastructure without an incident trail" instinct. Docs (Phase 6) carry the contract for now; if async creep ever appears, the rule earns its keep then and lands with a `CHU` code at that point — Phase 6 docs explicitly call out that path.
- 2026-05-25: Phase 6 (docs) shipped. `libraries/runner/README.md` adds `add_generator` to the "What's included" table, a new "Generator helpers (opt-in sub-module)" sub-table, a "Generator-driven sequential I/O" section with worked example + "when to pick which" framing + "what the runner does NOT use" subsection, and `generator_basic.py` to the examples table. `libraries/runner/docs/guide.md` mirrors the same generator-section addition + "choosing between add and add_generator" comparison table + "what the runner does NOT use" rationale (the four ranked reasons). `docs/contributing/style-guide.md` gains a "Cooperative concurrency" section that names the async / asyncio ban as a hard rule for contributors; AGENTS.md line 65 widened from the original "no async/await in check()/handle()" (Decision 0080) to the full project-wide ban (Decision 0087) with the helper sub-module named inline. Workstream `Status:` flipped from `proposed` to `shipped`.
