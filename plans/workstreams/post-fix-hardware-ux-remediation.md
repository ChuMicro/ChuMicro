...
Status: **shipped 2026-06-13 — Phases 0 + A–D complete.** Phase 0 (coverage gap) verified WS-1 / HTTP-3 / HTTP-1 clean; A (library correctness L1 + L2); B (workbench failure handling); C (workbench coaching); D (hardware-reality docs). Only pending item: real-board validation of the runner tick change (L2) folds into the standing RUN-1 runner-tick bake. Full evidence for every finding ID lives in the research artifact: [`plans/reviews/2026-06-13-post-fix-hardware-ux-review.md`](../reviews/2026-06-13-post-fix-hardware-ux-review.md).
...
**Shipped 2026-06-13 (mqtt 0.16.9, requests 0.13.9, msgpack 0.1.32).** No behavior change.

- **H1** (MEDIUM, docs) — the connect-blocks-the-reactor reality is now honest where users read it. The mqtt guide's TLS section already carried the synchronous-handshake fact + bench numbers but framed it as "the listener stalls" and the self-heal section oversold "without blocking the runner"; both now say the *whole reactor* is frozen for the handshake duration (seconds on a bad link), plus the DNS-blocking phase, with "connect before starting time-critical services" guidance. The requests guide + the `HttpClient` and `MQTTClient.connect` docstrings carried the same overclaim ("LED keeps blinking ... in a TLS handshake", "across multiple ticks rather than blocking") — corrected to name the per-runtime reality (TCP per-tick; DNS and MP/CP TLS handshake block). WIFI-2 needed no change: `wifi/config.py` already documents `connect_timeout_ms` as CircuitPython-only with the MP adapter ignoring it. *verified.*
- **L3** (MEDIUM, `msgpack/_pure.py` docstring) — made the trusted-only disclaimer load-bearing rather than adding a speculative `max_items` knob for a use the library does not support. `unpackb`'s docstring now states the concrete breadth amplification (an N-byte array/map payload can allocate an ~8N-byte container on a 256 KB board) and says not to `unpackb` an unbounded peer-supplied blob without bounding its length. *resolved.*
...
- 2026-06-13 — Phase D shipped (mqtt 0.16.9, requests 0.13.9, msgpack 0.1.32): H1 (connect-blocks-the-reactor honesty in the mqtt + requests guides and the connect docstrings; WIFI-2 already documented), L3 (msgpack `unpackb` docstring breadth-amplification caveat instead of a speculative knob). Docs/docstring-only, no behavior change. Workstream complete; only the runner-tick real-board bake remains (folds into the pending RUN-1 bake).
