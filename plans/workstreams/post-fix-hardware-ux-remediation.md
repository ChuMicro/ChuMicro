...
- **L1** (HIGH, `libraries/logging/src/chumicro_logging/core.py:124`) — `Logger.log`'s `message % args` interpolation sits outside the `try/except` that wraps `handler.emit`, so a format/arg mismatch raises into the caller and breaks the class's documented "logging never crashes the app" contract. Guard the interpolation; fall back to a non-crashing rendering (e.g. `f"{message!r} % {args!r} (log-format error: {error})"`). Add a test for the mismatch case (zero-arg `%`-literal already safe via `if args:`). VERSION patch bump. *verified.*
- **L2** (MEDIUM, `libraries/runner/src/chumicro_runner/core.py:402-405,446-462`) — RUN-1's `except Exception` swallows the re-entrancy guard's `RuntimeError`, so a handler that re-enters `tick()` gets a silent `handler_errors` bump instead of a loud crash. Give the guard a dedicated subclass and re-raise it past the isolation. Decide internal-vs-exported (exported → minor bump; the re-entrancy error is arguably worth being catchable by callers). Add a test asserting a re-entrant `tick()` propagates, not counts. *verified.*
