...
QoS-1 flood bake-validated 2026-07-04 (two runs, 100 + 1801 messages — see the
validation table); the workstream is CLOSED with no open residuals.  Several divergence
...
| 2026-07-04 | mqtt 0.24.0 (Decision 0104 rate convergence), QoS-1 flood bakes ×2, Pico W MP | **PASS — the guard is unreachable under a healthy flood.**  Burst run: 100 QoS-1 messages at 40/s; sustained run: 1801 messages over 45 s (`scripts/mqtt_qos1_flood.py`, 32 B payloads, subscribing `mqtt_bake_diag` harness).  Both runs: board received every message with `inbound_gaps 0`; zero handler faults, zero state transitions (never left CONNECTED — the pre-0104 behavior was a guard-fault reconnect cycle); broker log shows full PUBACK parity (100/100 and 1801/1801) with **zero DUP redeliveries**; the board's own 1 Hz publish cadence held exactly (`sent 297 puback_total 297` over the 300 s bake) — the coalesced ack batch demonstrably never consumes the send slot; and the end-of-bake heap was **byte-identical across the two runs** (130,736 free), so an 18× inbound-volume difference produced zero fragmentation drift.  With this row, every landed convergence step is hardware-validated and the workstream has no open residuals. |
