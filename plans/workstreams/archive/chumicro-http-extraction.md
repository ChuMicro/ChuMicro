...
- **Deployment overlap is rare.**  256 KB / 2 MB-flash (~800 KB usable) class boards typically run one of {`chumicro-requests`, `chumicro-http-server`, `chumicro-websockets`} at a time.  When two land together, the redundant ~80 LOC compiles to ~1.5 KB of `.mpy`.  Trivial.
