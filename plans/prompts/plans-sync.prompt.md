...
2. **Check each file against reality.** For each file below, verify that it
   reflects the current codebase. Fix anything stale.

   | File | What to check |
   |---|---|
   | `plans/README.md` | Decisions range (e.g., 0001–0014), current planning set list |
...
3. **Don't update what hasn't changed.** Only touch files where the content is
   actually stale. Avoid churn for the sake of churn.

4. **Commit the sync.** A single commit is fine. Subject line should name what
   was synced (e.g., "Update planning docs for Decision 0014 and serviceable library").

## Common staleness patterns

- New decisions not listed in prompt decision lists
