...
4. `libraries/runner/` provides:
   ...
   - 100% test coverage
5. `libraries/compat/` provides cross-runtime polyfills (`chumicro-compat` 0.1.0): `functools.partial` with a pure-Python fallback for MicroPython/CircuitPython.
6. `libraries/msgpack/` provides cross-runtime MessagePack serialization (`chumicro-msgpack` 0.1.0): `packb`, `unpackb`, `pack`, `unpack`. Delegates to CircuitPython's native C `msgpack` when available; otherwise uses a pure-Python encoder/decoder.
