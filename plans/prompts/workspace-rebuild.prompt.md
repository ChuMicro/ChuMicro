...
4. `libraries/serviceable/` provides:
   - `Event`, `EventQueueSink`, `SimpleEventDispatcher`, `ServiceRunner`
   - `FakeEventSink` in `testing` submodule
   - 100% test coverage
5. `scripts/run.py` provides all task entrypoints with auto-discovery:
