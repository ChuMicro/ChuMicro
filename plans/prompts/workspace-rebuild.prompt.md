...
- `support/test_harness/` and `support/docs/`
...
│   ├── docs_deploy.py      # docs deployment helpers for GitHub Pages
│   └── generate_landing_page.py  # root landing page for GitHub Pages
├── .github/workflows/
│   ├── ci.yml              # lint, test, build, verify-examples, docs-build, version-check, api-check, MP/CP compat
...
│   ├── labels.yml
│   └── skills/             # agent skill instructions (git-commit, large-output, run-script, terminal-recovery)
├── AGENTS.md
├── LICENSE                 # MIT license (required by bundle workflow)
...
│   └── workstreams/
├── support/
│   ├── docs/              # shared docs assets (extra.css, favicon.png)
│   └── test_harness/
└── libraries/
    ├── timing/
    │   ├── pyproject.toml
...
1. `support/test_harness/` provides a minimal `run_module()` runner for `functional_tests/`.
2. `libraries/timing/` provides:
...
3. `libraries/runner/` provides:
   - `TaskHandle`, `Runner` (gate-based service with shared timestamps and batch firing)
   - `CallRecorder` in `testing` submodule
   - 100% test coverage
4. `libraries/compat/` provides cross-runtime polyfills: `functools.partial` with a pure-Python fallback for MicroPython/CircuitPython.
5. `libraries/msgpack/` provides cross-runtime MessagePack serialization: `packb`, `unpackb`, `pack`, `unpack`. Delegates to CircuitPython's native C `msgpack` when available; otherwise uses a pure-Python encoder/decoder.
6. `scripts/run.py` provides all task entrypoints with auto-discovery:
   - `setup`, `sync-ide`, `new-library`, `lint`, `test`, `build`, `preflight`
   - `verify-examples`, `docs`, `docs --serve`, `docs-preview`, `docs-deploy`
...
7. `conftest.py` at root auto-discovers source roots and excludes `functional_tests/`.
8. `pyproject.toml` uses broad `testpaths`; `test` runs pytest per-library for multi-library isolation.
9. `.idea/chumicro.iml`, `.idea/runConfigurations/`, `pyrightconfig.json`, `.vscode/tasks.json`, and `.vscode/settings.json` are generated by `sync-ide`.
10. CI runs host checks (required) plus required MicroPython and CircuitPython runtime compatibility jobs.
11. Release pipeline publishes to PyPI (trusted publishing), creates git tags and GitHub Releases, stages bundle artifacts (.py + .mpy + package.json) to channel-specific bundle repos, and creates circup-format release zips.
12. `scripts/check_version.py` enforces VERSION bumps; `scripts/check_api.py` detects API breakages; `scripts/bundle.py` handles bundle staging and README generation.
