...
### 2026-04-25 — Phase 3b kick-off: `chumicro-kvstore` Slices 0–1 + Decision 0034

Phase 3b started.  Decision 0034 nails down the API and per-backend contracts the ADR 0030 deferred — mapping-shaped `KVStore` with three lifecycle methods (`commit`, `commit_if_changed`, `reload`), four exceptions (`KVStoreError`, `KVStoreFull`, `KVStoreCorrupt`, `KVStoreReadOnly`), single `KVStore` class with backend selected at construction (`auto` | `memory` | `nvm` | `nvs` | `littlefs`), CP NVM `MAGIC | LEN | CRC32 | MSGPACK` framing (10-byte header — leaves 246 B payload on SAMD21's 256 B NVM), MP NVS one-blob-per-key in namespace `chu_kv` with msgpack values, MP LittleFS single `/_chu_kv.msgpack` with tmpfile + rename atomicity, MemoryBackend as the CPython default + `FakeKVStore` substrate, msgpack codec inside `KVStore` so backends only deal in `bytes`.

Slice 1 lands the library skeleton: `libraries/kvstore/` scaffolded via `new-library`, `KVStore` class implementing the full mapping API + lifecycle methods + introspection properties, `MemoryBackend` complete, hardware-backend stubs (`CpNvmBackend` / `MpNvsBackend` / `MpLittlefsBackend`) raising `NotImplementedError` with explicit pointers to slices 2–4, `chumicro_kvstore.testing.FakeKVStore` with `simulate_corrupt` / `set_capacity` / `record_calls` hooks for downstream library tests.  43 host-side tests at 99.6 % coverage.  Pyproject depends on `chumicro-msgpack`; cross-runtime tests run clean on CPython + MP unix-port + CP unix-port.  Quickstart example rewritten as a boot-counter that survives reboot.  README quick-example + What's-included tables filled in; scaffolder TODOs cleared.

Slices 2–4 (per-runtime backends) land next on the four plugged-in boards (Lolin S2 CP/MP, Pi Pico W CP/MP).

### 2026-04-25 — chumicro-repl Phase 2 close + VS Code Testing-panel on-device validation + workbench release-workflow coverage + planning-ecosystem restructure
