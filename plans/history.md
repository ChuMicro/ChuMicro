...
**Support package rename:** `support/testing/` (`chumicro_testing`) renamed to `support/abstractions/` (`chumicro_abstractions`).  The package contains both production code (`RealTime`) and test fakes (`FakeTime`), so "abstractions" better describes injectable interfaces and their implementations.  `FakeTime` was first extracted from `device_transport/testing.py` into the shared package, then `RealTime` moved alongside it.

**Editable-install support packages:** `find_support_packages()` added to `workspace.py`.  `install_editable()` in `shared.py` now installs both publishable libraries and support packages.  Previously support packages relied solely on `conftest.py` `sys.path` manipulation.
