...
Codified the validation as `scripts/audit_gates.py` so future regressions of the same shape get caught locally — 12 scenarios covering check_names CHU001 (single-letter + abbreviation), check_whitespace CHU002 + CHU004, ruff F401 (plus a clean-passes counterpart), verify_examples (broken import), check_version (workbench src no-bump fail + with-bump pass), and check_api (workbench breakage + patch → fail / minor → pass, libraries breakage + patch → fail).  Runs in ~1.5 s end-to-end.  Self-validating: temporarily reverting the absolute-vs-relative `--search` fix made the three check_api scenarios fail as expected, then restoring brought all 12 back to green.  Surfaces one known gap inline: the AGENTS.md "absolute imports only in `libraries/*` + `support/test_harness/`" rule has no static pre-merge gate — it's enforced only at runtime when modules fail to import on device.  Not auto-run in CI by design (per earlier UX steer); invoke before structural `scripts/` changes or after upgrading griffe / ruff / pytest.
