...
Reference implementation: `.github/skills/regen-comments/wf_run.py` (`run_workflow` + `copy_winner`); callers `regen_phase2.py` / `regen_symbol.py` (full guard) and `regen_phase1.py` (prompt-only — its fresh-room precondition makes stale artifacts impossible and a zero-fact triage legitimately writes nothing). Corollary for single-shot `claude -p` writers (`tighten_symbol.py`): delete the expected output before the call so the existence check is honest.
