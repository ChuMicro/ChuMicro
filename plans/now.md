...
- **Phase:** **Multi-thing-staging replacement (scoped diff-deploy) — primitive shipped.**  Workspace-ecosystem umbrella main thread closed earlier this session.  This session's last unit of work: the transport-layer primitive that replaces the retired multi-thing staging path with a "scan + diff + delete + write" deploy.  New module-level helpers `is_in_deploy_scope` / `DEPLOY_SCOPE_FILES` / `DEPLOY_SCOPE_PREFIXES` define the managed surface (`/code.py`, `/main.py`, `/active.py`, `/runtime_config.msgpack`, `/lib/*`).  `TransportProtocol` gains `list_files_in_scope` + `delete_files`; MicropythonTransport implements via raw-REPL `os.listdir` walks (copy mode only), CircuitpythonTransport via direct CIRCUITPY-drive `pathlib` walks.  `Deployer.deploy_diff(source, ...)` orchestrates list → diff → delete → deploy_files; in RAM modes the listing returns empty so the routine collapses to plain `deploy_files`.  Out-of-scope files (settings.toml, photo.jpg, hand-edited boot.py) survive every diff-deploy.  19 new unit tests + CP flash + RAM hardware tests pass; MP flash hardware tests blocked by pre-existing Pi Pico W flash state (needs firmware reflash to recover, unrelated to this slice).
- **Last shipped:** scoped diff-deploy primitive — `Deployer.deploy_diff` + transport-level `list_files_in_scope` / `delete_files`.
- **In flight:** —
- **Blocked on:** —  Carry-over: a `--wipe` CLI flag that calls `storage.erase_filesystem()` (CP) / walk-and-delete (MP) is a separate slice once the primitive proves out on the field.
- **Last touched:** `workbench/deploy/src/chumicro_deploy/{__init__,protocol,deployer,micropython_transport,circuitpython_transport,testing}.py`, `workbench/deploy/tests/{test_diff_deploy,test_micropython_transport,test_circuitpython_transport}.py`, `workbench/deploy/functional_tests/test_diff_deploy_hardware.py`, `plans/now.md`.
