...
- **Last shipped:** **chumicro-deploy: simplify `CircuitpythonTransport.disconnect` to pure teardown.**  Removed `_restore_autoreload()` and the autoreload-on call from `disconnect()`.  Rationale: it was redundant on the `deploy_files` path (the mid-method Ctrl-D soft-reboot resets `supervisor.runtime.autoreload` to default-on as a side effect) and unnecessary on the `_stage_to_flash` path (functional tests drive raw REPL directly; `code.py`-style reload-on-edit semantics aren't relevant during or after a test session).  Disconnect is now just re-enter raw REPL → Ctrl-B → close port; the re-enter is kept for the kill-mid-execute case (Ctrl-C × 2 interrupts running code).  Inverted `test_flash_disconnect_restores_autoreload` into a regression guard against re-introducing the explicit restore (which would re-create the ESP32-S2 USB-CDC double-reboot wedge).  Refreshed disconnect / `_disable_autoreload_before_drive_writes` / `disconnect_all` (pytest-device) docstrings + `plans/learnings.md` "rsync to CIRCUITPY can hang" entry to match.
- **In flight:** idle (this session).
- **Blocked on:** —.  (Preflight is currently red because of in-flight cross-runtime test conversions by a concurrent agent in `libraries/logging/tests/test_logging.py` + `libraries/mqtt/tests/test_packets.py`; not this session's work, will clear when the other agent commits.)
- **Last touched:** workbench/deploy/src/chumicro_deploy/circuitpython_transport.py, workbench/deploy/tests/test_circuitpython_transport.py, workbench/pytest-device/src/chumicro_pytest_device/plugin.py, plans/learnings.md, plans/now.md.
