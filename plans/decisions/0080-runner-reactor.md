...
### Register the socket object, not a file descriptor

A socket-owning service registers its socket with the runner's poller via `watch(sock, read=, write=)` and `unwatch(sock)` at lifecycle transitions (connect, close, the read-to-write flip), which are rare events, not per-tick work. The runner registers the **object**: `fileno()` is `-1` or absent on CircuitPython radio sockets and rp2 MicroPython sockets, but the object is pollable through the runtime's stream `ioctl` on all three runtimes. `chumicro-sockets` exposes an accessor returning the underlying pollable socket, since its wrappers are plain Python objects the poller cannot register directly.
...
- `chumicro-runner` gains `wait` / `watch` / `unwatch`, a min-deadline scan, and an optional `next_deadline` read. A heap-drift test must show zero steady-state allocation in the wait path (Decision 0051's tracemalloc standard).
- `chumicro-sockets` gains an accessor returning the underlying pollable socket.
- Long-lived I/O services (`requests`, `mqtt`, `websockets`, `http_server`) add `watch`/`unwatch` at connect/close and an optional `next_deadline`. Additive, no API break.
