...
- **Adopt the asyncio module as the scheduler.** Its I/O reactor is coroutine-bound (readiness reschedules the `await`-ing task, so no coroutine means no I/O sleep), and its C `TaskQueue` is an intrusive heap that blind-casts pushed objects to the `Task` struct, so neither piece can be borrowed without going all-in on the module's scheduler. The wait we actually need is `select.poll().ipoll`, a C call we already make directly, and with 3 to 10 services a linear `min(next_due_ms)` scan replaces the task heap. Re-affirms Decision 0014 and 0051 on shape, now with the C-engine evidence. Sequential I/O state machines that read more naturally as linear code than as `check`/`handle` are expressed as generator functions registered with the runner via `runner.add_generator()`, with the runner driving the generator via `.send()` and our own wait-tokens — the same posture trio and curio take on CPython ([Decision 0087](0087-generators-for-sequential-io.md)). The `async`/`await` keywords and the asyncio module are both banned in library code.
