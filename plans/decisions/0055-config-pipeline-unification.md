...
3. **Mono-repo workspace root config.** `workspace.yml` lives at the mono-repo root, gitignored, carrying defaults + credentials in one place (Decision 0057).  The mono-repo's `_workspace_template/workspace.yml` carries this repo's specific opinions (`mqtt.broker.host = test.mosquitto.org`, `wifi.ssid` placeholder); setup materialises it to the workspace root on first run, otherwise the workbench-owned starter (`chumicro_workspace.read_workspace_yml_starter`, sourced from `_payloads/workspace_yml/starter.yml.template`) is the fallback.  `read_devices_yml_starter` follows the same workbench-payload pattern (single source of truth shared between mono-repo and workspace-template).
