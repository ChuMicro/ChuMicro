...
Guides should be generated by an AI agent using the prompt at `plans/guide-generation.md`. This prompt tells the agent which source files to read and which sections to produce. Human contributors may also write guides by following the same structure.
