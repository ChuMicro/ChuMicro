...
Guides should be generated by an AI agent using the `guide-generation` skill (`.github/skills/guide-generation/SKILL.md`). This tells the agent which source files to read and which sections to produce. Human contributors may also write guides by following the same structure.

The scaffolder template (`_GUIDE_TEMPLATE` in `scripts/new_library_scaffold.py`) includes generation instructions that point to this prompt. A library must not be released with placeholder comments still in its guide.
