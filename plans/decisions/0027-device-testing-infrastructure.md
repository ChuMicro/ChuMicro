...
publishable `chumicro-deploy` package (Decision 0032).  The protocol
is captured as a
...
A pytest plugin (`chumicro-pytest-device`, in `workbench/pytest-device/`, registered via `pytest11` so it auto-loads when the package is installed) intercepts explicit `functional_tests/` targets.  Each collected `test_*` function becomes a pytest item that stages, executes on device, parses output, and reports as pytest pass/fail.  **No environment variable setup is required** — `devices.yml` is the gate.
...
6. Production deploy (`deploy_files`): exit raw REPL, send Ctrl-D — single
   ...
   USB-CDC double-reboot wedge surfaced.  See
   [`docs/troubleshooting/macos-circuitpy.md`](../../docs/troubleshooting/macos-circuitpy.md)
   for the failure mode and recovery chain.
...
A `result_parser` module (`chumicro_pytest_device.result_parser`, shipped as part of the `chumicro-pytest-device` workbench package) parses the harness's structured output (`PASS`, `FAIL`, `SKIP`, `SUMMARY`, `HEAP` lines) into typed result objects.  This is the contract between device execution and host reporting.
