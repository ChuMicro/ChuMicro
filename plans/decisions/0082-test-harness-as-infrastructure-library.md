...
Status: `accepted`
Date: `2026-05-24`
Summary: Promote `chumicro_test_harness` from `support/` to `libraries/test_harness/` as an infrastructure library (`kind = "infrastructure"`); snapshot-shipped, browse-hidden; decided, not yet built.
...
**`chumicro_test_harness` is promoted from `support/` to `libraries/test_harness/` as an infrastructure library: it ships through the same snapshot channel as every other cross-runtime library, follows the same shape rules, and is hidden from the browsable catalogue unless asked for by name.**

**Implementation status.** This promotion has not landed.  As of this writing `chumicro_test_harness` still lives at `support/test_harness/`, there is no `libraries/test_harness/`, its `pyproject.toml` carries no `kind` field, and `library browse` / `list` have no `kind` filter.  What *has* shipped in place is the `chumicro_test_harness.network` submodule (below) and the migration of the networking libraries' functional tests onto it; the tree move and the `kind`-filter plumbing are the unbuilt remainder, tracked as Phases 4–5 of [`plans/workstreams/test-harness-promotion-and-network-helper.md`](../workstreams/test-harness-promotion-and-network-helper.md).  The trigger for finishing it is a workspace-template consumer that pulls a networking library via `library add` and needs to run that library's functional tests or examples without the mono-repo on disk — until such a consumer exists the harness stays mono-repo-local under `support/`, and the decision is honored today only by its import-name invariant (below).

- **Tree placement.** Source moves to `libraries/test_harness/src/chumicro_test_harness/` and the `support/` tree is then dissolved (no other entries justify it; future infrastructure packages land in `libraries/` with the same flag).
- **Infrastructure flag.** `pyproject.toml`'s `[tool.chumicro]` block gains `kind = "infrastructure"`.  The snapshot index records the kind; `chumicro-workspace library browse` and `library list` filter `kind == "infrastructure"` entries by default.  `list --all` shows them.  `library add <name>` (explicit ask) and `fetch_closure` (transitive resolve) ignore the flag — the flag controls *discovery*, not *fetchability*.
