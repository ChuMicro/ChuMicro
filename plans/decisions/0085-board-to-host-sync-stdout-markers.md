...
- The `chumicro_workspace.markers` module owns the marker queue + the `wait_for` primitive that host fixtures call. Thread-safe (queue.Queue under the hood); the backend's reader thread pushes, the host main thread waits. Lives in workspace (not pytest-device) because it's also consumed by `chumicro_workspace.deploy_api` for demos — see [Decision 0086](0086-programmatic-deploy-api-for-demos-and-tooling.md).
...
- The marker syntax is documented at the head of `chumicro_workspace.markers` so a contributor adding a new marker has one place to read the rules.
