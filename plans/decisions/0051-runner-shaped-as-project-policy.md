...
**The `asyncio` module / event loop as the cross-cutting scheduler.**  Not a support claim — all three runtimes ship a usable `asyncio` module on the boards we target.  The rejection is that an event loop is the wrong *shape* for this class of device, on two grounds:
...
- **The non-blocking guarantee doesn't survive the asyncio socket layer.**  The whole point is "nothing blocks the main loop."  But MicroPython's asyncio still resolves names through a blocking `getaddrinfo` (documented in its own asyncio reference), so an `await`-ed connect can still stall the loop — the project would have to layer the same non-blocking-DNS / chunked-yield machinery the runner already provides *on top of* asyncio to get the guarantee.  The event loop adds a scheduler whose task ordering is less predictable, for no net gain over the explicit tick.  Adafruit's CircuitPython port carries an additional broken-on-CP stream layer ([issue #4](https://github.com/adafruit/Adafruit_CircuitPython_asyncio/issues/4), open since 2021-11) that compounds this.

Decision 0014 rejected this; we re-affirm on shape, not capability.  [Decision 0087](0087-async-await-syntax-without-asyncio-module.md) refines the boundary: the asyncio module remains banned in `libraries/` / `support/` / `workbench/`, but the `async`/`await` keywords are a compiler feature independent of the module and are permitted inside coroutines registered via `runner.add_coroutine()` — the runner drives the coroutine via `.send()` with our own wait-tokens, the same architectural posture trio and curio take on CPython.
