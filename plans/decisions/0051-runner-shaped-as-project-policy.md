...
Decision 0014 rejected this; we re-affirm on shape, not capability.  [Decision 0087](0087-generators-for-sequential-io.md) refines the boundary: the asyncio module and the `async`/`await` keywords are both banned in `libraries/` / `support/` / `workbench/`, but sequential I/O state machines may be expressed as generator functions registered with the runner via `runner.add_generator()` — the runner drives the generator via `.send()` with our own wait-tokens, the same architectural posture trio and curio take on CPython.
