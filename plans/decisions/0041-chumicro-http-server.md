...
`chumicro-websockets` (Decision 0045) made the same inline call when it arrived as the third HTTP/1.1 consumer — its handshake encoder carries a slim copy of `CaseInsensitiveDict` + `CRLF` rather than importing them.  Extracting into a shared `chumicro-http` package was re-evaluated 2026-05-12 and declined: ~150 LOC of byte-equivalent primitives is duplicated across three `_wire.py` files totalling ~3,110 LOC, most boards deploy only one of the three consumers, and the one primitive-level divergence to date (`_order` insertion-order tracking missed during websockets's dict-slimming, fixed in commit `63774d72`) was caught by `/audit-integration` ~13 days after it landed with no user-visible impact.  RFCs 7230 §3.2 / 7231 §3.1.1.5 are stable; primitive churn is low; `/audit-integration` is the working safety net.  Revisit only if primitive-level changes start outpacing the audit cadence, or a fourth HTTP/1.1 consumer arrives.  Full reasoning in [`workstreams/archive/chumicro-http-extraction.md`](../workstreams/archive/chumicro-http-extraction.md).
...
* Decision 0040's "extract shared HTTP wire primitives" follow-up: declined.  `chumicro-websockets` (Decision 0045) made the same inline call when it arrived as the third HTTP/1.1 consumer.  Re-evaluated 2026-05-12 against actual cost/benefit and the inline position held — `/audit-integration` keeps the three copies in sync (the one divergence to date was caught with no user-visible impact), and a ~150 LOC duplication across three `_wire.py` files doesn't justify a new package plus version-coupling across three consumers.  See §5 for the standing trigger and [`workstreams/archive/chumicro-http-extraction.md`](../workstreams/archive/chumicro-http-extraction.md) for the full reasoning.
