...
- `promote.yml` is a `workflow_dispatch` that triggers `release.yml` with `channel=stable` for named libraries, then triggers `deploy-documents.yml` with `channel=stable`.
- `deploy-documents.yml` triggers on push to `main` (experimental docs) and via `workflow_dispatch` with `channel=stable` (stable docs, called by `promote.yml`).
