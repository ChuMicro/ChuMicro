...
- **Hard breaking change; no compat shim.** The retired `chumicro_workspace.secrets` resolver isn't replaced — there is no code path that still reads `secrets.yml` or interprets `!secret <name>` markers.  A pre-0057 workspace that upgrades `chumicro-workspace` to 0.8.0 without migrating its config files breaks at the next deploy: any `password = "!secret wifi_password"` string in a committed `config.toml` lands on the device verbatim, the wifi adapter tries to authenticate with the literal text, and the connect attempt fails.  Migration is a paste per workspace: drop every `!secret` reference from `workspace.yml` / `config.toml`, and shape the values from the old `secrets.yml` into the new `workspace.local.yml` (`wifi_password: foo` becomes `defaults: { wifi: { password: foo } }`).  Both repos are sole-developer at the time of this change; the no-shim trade-off is acceptable for that audience.
