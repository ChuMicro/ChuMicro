...
projects/back-porch/
  app.py
  config.toml              # project-level defaults, checked in
projects/back-porch/_generated/
...
Wifi creds live in the config pipeline above, not in any persisted-state store.  Credentials rotate at redeploy, travel with the gitignored `workspace.yml` (Decision 0057 retired the older `secrets.yml` + `!secret` design in favour of one gitignored file), and never touch NVM / NVS / filesystem as mutable values.
