...
- Per-backend sizes and semantics are pinned from source reading (CP `mpconfigport.h` per port, `esp32_nvs.c`); no on-device verification is planned.  Implementation surfaces the constants via a `KVStore.capacity` property, and cross-runtime unit tests on the unix-ports cover behavior up to what host testing can prove.
