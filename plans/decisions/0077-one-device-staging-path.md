...
- The invariant is in force on the CircuitPython path: a default `chumicro-workspace deploy` is clean-slate (whole-drive diff minus the keep set), `--no-wipe` is the additive opt-out, `--wipe` the full-erase escape, and a board `settings.toml` is evicted with a one-time notice — bench-verified on a Pi Pico W. Decision 0059 §1's separate `example_source` framing is superseded and 0059 §1 is edited in place to cross-link here. The MicroPython keep-set survive-set (next bullet) is the one remaining mechanic; the decision is accepted regardless (per `decisions/README.md`, incomplete implementation does not keep an ADR `proposed`).
...
- MP keep-set mechanic (implemented): `lfs mkfs` has no `--exclude`, so the keep set is preserved via a *scoped delete*, never a blind reformat — `_clean_slate_device` removes every device-root entry except `{boot.py, boot_out.txt, _chu_kv.msgpack}` then `fs cp` repopulates the payload, and the MP diff scope (`list_files_in_scope(clean_slate=True)`) walks the whole device minus the keep set so the additive `fs cp` no longer strands stale root files.
