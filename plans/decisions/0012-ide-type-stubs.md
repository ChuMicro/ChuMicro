...
The same file is read by `scripts/prepare.py` for cloning.  Changing the version in one place updates stubs, cloned repos, and built binaries on the next `setup` run.
