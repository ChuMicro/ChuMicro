...
**Reactive setup stays callbacks; MQTT's inbound gained the stream flavor.** MQTT's `connect` stays reactive — Decision 0091's `Signal`/`wait_for` (which post-dates the original form of this decision) gives any app a linear connect-wait in three lines with zero mqtt API, so a dedicated `wait_connected()` would be sugar. `publish` / `subscribe` stay fire-and-forget queueing (QoS acking is internal bookkeeping the caller never blocks on). Inbound, however, is flavor 2 exactly: `MQTTClient.next_message()` (mqtt 0.20.0) mirrors the websockets design — bounded drop-oldest queue, first use flips data delivery from `on_message`/pattern handlers to the queue, lifecycle callbacks unaffected — for single-subscription consumers; multi-topic fan-out stays on the callbacks. `chumicro_http_server` is out of scope: route decorators are the idiomatic server shape; only streaming response bodies would use generators, alongside the deferred Decision 0081 Phase 6 TLS-accept work.

## Consequences

- New public surface: `chumicro_requests.generators` (opt-in submodule), and `next_message()` + a bounded-inbound-queue knob on `chumicro_websockets` (`InboundMessage`, `max_inbound_queue_size`) and `chumicro_mqtt` (`InboundPublish`, `max_inbound_queue_size`, added 2026-07-03). Both reuse the existing connector-factory injection; neither replaces the `check`/`handle` client, which remains for repeated requests / callback delivery.
