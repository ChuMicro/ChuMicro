...
|---|---|---|---|
| `workspace.yml` | gitignored at the workspace root; materialized by `setup` from the canonical template (`chumicro_workspace.read_workspace_yml_template`) | **Workspace machinery** — `library_sources` (dev-mode editable overrides), `deploy_targets` (per-project → device mapping), `quality` (lint / coverage knobs), future `environments` block. | **No.**  Host-only. |
