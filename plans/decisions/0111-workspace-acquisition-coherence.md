...
- Two coherence questions stay open and move to [`plans/open-questions.md`](../open-questions.md): a release identifies the same snapshot three ways (PyPI semver, bundle date tag, libraries timestamp tag) with nothing correlating them, and the device bundle hard-codes one MicroPython and one CircuitPython mpy format with no scheme for the next ABI version.
