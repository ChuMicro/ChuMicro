...
The renamed `_subcommand_phase_factory` now sets `CHUMICRO_RAW_OUTPUT=1` in the child's environment so the child's own dispatcher resolves to `_RawDispatcher` (raw passthrough); without this, the child would build its own dispatcher and the parent would see nested `[label] [inner-label] line` framing.  The previous name `_preflight_phase_subprocess_factory` is kept as a backward-compat alias.
