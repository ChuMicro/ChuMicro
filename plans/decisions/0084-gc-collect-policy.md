...
Status: `accepted`
Date: `2026-05-24`
Summary: `gc.collect()` is forbidden in hot paths, required at the end of library `__init__.py` files building substantial top-level state, allowed between state changes (post-handshake, post-bulk-decode).
...
- **Required after substantial import-time state.** Any library `__init__.py` whose top-level imports build substantial state (parsers / decoders, multi-class setups, runtime-specific adapter loads, module-scope `const()` tables) ends with:

  ```python
  import gc as _gc; _gc.collect(); del _gc
  ```

  Heavy multi-file libraries (mqtt, websockets) also `_gc.collect()` between submodule imports inside `__init__.py`. Tiny single-purpose libraries (a polyfill, a logger config, one small dataclass) don't earn the boilerplate.

- **Allowed between state changes.** Post-handshake, post-bulk-decode, post-`connect()` — places where a pre-emptive collection beats triggering inside the next allocation (which would steal time from that allocation under pressure). The library author marks these with a comment naming the reason.
...
- New libraries with substantial import-time state add the `_gc.collect()` boilerplate at scaffold time.
