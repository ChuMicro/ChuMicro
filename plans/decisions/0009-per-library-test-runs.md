...
`scripts/run.py test-host` runs a separate pytest subprocess for each package that has a `tests/` directory. Coverage data from each run is written to a per-library file (`.coverage.<name>`), then combined with `coverage combine` and reported once at the end.
...
4. The 90% coverage gate is enforced only on the final combined report, not per-run.
...
- Bare `pytest` from the repo root is no longer the supported path; use `python scripts/run.py test-host`.
