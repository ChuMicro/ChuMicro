...
Library code expresses sequential I/O state machines as **generator functions** registered with the runner via `runner.add_generator(gen) -> GeneratorHandle`. The generator suspends by `yield`-ing a duck-typed wait object — anything exposing `io_socket` / `io_interest(now_ms)` / `next_deadline(now_ms)`, or the clock-free predicate `ready(now_ms)` added by [Decision 0091](0091-event-wait-tokens-for-generator-tasks.md). The runner reads those each `wait()` to register the socket with ipoll, and resumes the generator via `.send(now_ms)` once the socket is ready, the deadline elapses, or the event fires. The runner's external dispatch surface is unchanged — internally the wrapper satisfies the same check/handle/io_* contract everything else does.
...
- **A yielded wait publishes its condition and never judges a time itself.** `ready(now_ms)` answers only what needs no clock (a `Signal` that has been set); `next_deadline(now_ms)` publishes a deadline; `io_socket` + `io_interest(now_ms)` publish poll interest. Whoever drives the generator owns the clock and does every comparison, so a task registered on a `Runner(ticks=…)` has its deadlines measured in the caller's units — a wait that reached for `chumicro_timing` directly would silently override an injected clock, and a `chumicro_timing`-shaped fake would agree with it in tests. `_GeneratorWrapper` therefore takes the runner's `ticks_diff` at construction rather than importing one. Waits are **not** awaitables — no `__await__`. They are cacheable: an EAGAIN-loop helper constructs one wait outside the loop and re-yields it, so steady-state iterations allocate nothing.

- The resumption gate is three ordered cases: honour `ready` when it answers true, else resume once `next_deadline` lands, else resume on any pass. Keeping it that small is what makes these helpers drivable from a loop that is not a `Runner` at all, which is the adoption path Decision 0042 and [standalone integration](../../docs/contributing/standalone-integration.md) exist to keep open; that page and the runner guide both carry a copy-paste implementation.
