...
- **Amended 2026-08-17 (`chumicro_runner` 0.22.0, `chumicro_timing` 0.9.0): `ready(now_ms)` is the resumption gate, and the rest are sleep hints.** The wait protocol had grown four duck-typed shapes reconciled only inside `_GeneratorWrapper.check()`, which made `runner.add_generator` the only driver that could get the timing right. That taxed the adoption path Decision 0042 and the standalone-integration guide exist to keep open: a reader who lifts `chumicro_sockets.generators` into their own loop should not have to reimplement a scheduler's private gate. The rule is now: a wait that must hold a generator back answers `ready(now_ms)`; a wait without `ready` may be resumed on any pass; `io_socket` / `io_interest` / `next_deadline` are optional hints that let a scheduler park the CPU. Two changes made it true, and the audit found no others were needed. `chumicro_runner.generators.sleep_until` enforces its own deadline (looping on the `now_ms` the driver sends back, re-yielding one cached wait) instead of trusting the driver to honour `next_deadline`; it was the single helper that returned short under a naive driver. `chumicro_timing.waits.Signal.ready` now also opens once a bounded wait's deadline has passed, so `wait_for` gets resumed and raises `ETIMEDOUT` under any driver. Socket waits deliberately keep no `ready`: their helpers retry on `EAGAIN` and re-suspend, so "resume on any pass" is already correct and adding a per-tick predicate call would cost the hot path for no behaviour change. Behaviour under `Runner` is unchanged, and `_GeneratorWrapper.check()` keeps its fallback for waits that predate the rule. The runner-less driver is now a documented recipe with test coverage (`libraries/runner/tests/test_socket_generators.py`, the "without a runner" cases).
