...
- **Class docstring:** one sentence.
- **Function/method docstring:** one summary sentence. Then `Args:` / `Returns:` / `Raises:` when earned. Skip the sections on dunders and trivial passthroughs.
...
- **No invented-metaphor class labels.** Don't compose engineering metaphors nobody uses outside the docstring (`Heartbeat gate`, `Wrap-safe primitive`, `Periodic-event gate`). Common English nouns that name what the class is or does (`helper`, `check`, `source`, `clock`) are fine when accurate. Describe what the class does for a caller using actual verbs; reach for a noun only when it earns the slot.
...
- **No two-fact summaries that hide load-bearing facts behind punctuation.** Don't write `Hand-driven ticks source for tests: starts at start_ms and only moves when advance() is called.` — the colon splices behavior past punctuation a skim-reader misses. Pick the more important fact, drop the other; if both are essential, the second goes in `Args:` / `Returns:`. **Acceptable forms** that don't hide facts: `category: items` module summaries (`Test-support helpers: a sleep_ms shim and a FakeTicks.`), or `;` joining two closely-related observations (`Hand-driven clock; ticks_ms() only changes when advance() is called.`). The rule is about hidden compression, not about punctuation.
- **Module docstrings: verb-led where possible.** Open with a verb describing what the module does, not a noun phrase labeling what's inside (`Wrap-safe millisecond ticks.` beats `Millisecond clock and signed-difference math:`). A noun-phrase label is acceptable only when no verb captures the module's role. **For `__init__.py` re-export modules:** `Exports <symbols>` is the preferred opener — clearer than `Re-exports`. Both are acceptable, but default to `Exports` unless the module does processing beyond exporting.
