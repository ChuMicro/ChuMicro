...
- No AI-tic words: `canonical`, `idempotent`, `comprehensive`, `seamless`, `robust`, `cutting-edge`, `leverage`, `intuitive`, `elegant`, `streamlined`, `battle-tested`, `first-class`, `out of the box`, `dive into`, `under the hood`, `magic`, `powerful`. The `shape` ban covers the bare word AND every `X-shaped` compound — `blob-shaped`, `disk-shaped`, `L-shaped`, `T-shaped`, `bell-shaped`, `tree-shaped`. If you find yourself writing `X-shaped`, the noun before `-shaped` is doing the real work and the `-shaped` suffix adds nothing. Drop the suffix and use the noun directly, or describe the actual structure. `shape` itself usually swaps to `layout`, `structure`, `behavior`, `pattern`, or `type`.
