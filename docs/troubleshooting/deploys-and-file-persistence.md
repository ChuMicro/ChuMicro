...
The first week with the deploy tool has two traps that both look like lost data: a default deploy clears files you put on the board by hand, and RAM mode (a deploy mode that runs your code straight from host memory) never writes anything to the board at all.  This page covers both, along with the memory error you hit when a RAM-mode deploy is too big for the board.
...
The board is in RAM mode.  RAM mode mounts your host source as `/remote/` on the device and never writes the board's flash or non-volatile memory, so absolute paths like `/runtime_config.msgpack` and any state a previous run saved simply aren't there.  Anything that counts across reboots or reads a file at a fixed absolute path fails.  (background: [Decision 0047](../../plans/decisions/0047-deploy-mode-flash-default.md))

Switch the device to flash mode, which writes files to the board's own filesystem.  Set it per device in `devices.yml`:
...
Deploy in flash mode, which writes to the filesystem instead of loading the whole payload into memory:
