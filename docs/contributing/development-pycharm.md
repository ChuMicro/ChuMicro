...
For device-backed `functional_tests/`, the test tree includes synthetic nodes such as `Setup — MicroPython`, `Batch run — MicroPython`, `Setup — CircuitPython`, and `Batch run — CircuitPython`. Those nodes own the staging and batched file-execution time so the first real test function does not appear artificially slow.
