...
For device-backed `functional_tests/`, the test tree includes synthetic runtime batch nodes such as `[micropython batch]` and `[circuitpython batch]`. Under each batch node, PyCharm shows `setup / stage device`, `run file`, and the parsed test results for that runtime. This keeps staging and batched file-execution time off the first real test function.
