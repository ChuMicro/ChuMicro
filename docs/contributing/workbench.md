...
- Promotion to **stable** publishes the base name (`chumicro-deploy`) — same `promote.yml` flow libraries use.
...
- **No cross-runtime tests.**  Workbench packages target only CPython — `test-micropython` and `test-circuitpython` don't apply.
