...
- **Curate code listings for the page's reader.**  A front-page listing carries only lines its reader can follow; expert options (last wills, retained state echoes, tuning knobs) stay in guides and demos even when a real deployment would use them.  If a listing needs three paragraphs of explanation, shrink the listing.
...
- **Re-derive, don't re-word.**  When review says a passage misses its audience, rewrite the section from what that reader needs, usually code and prose together.  Swapping synonyms and sentence order inside the wrong frame keeps the frame.
