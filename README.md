...
- **Bring your own socket, your own clock, your own fakes.**  Each library asks for its I/O dependencies when you construct it.  `MQTTClient` asks for a socket; `Heartbeat` asks for a clock; you decide what to pass.  ChuMicro provides defaults (`chumicro-sockets`, `chumicro-timing`) so the simple setup just works.  But you can hand in a stdlib socket for a desktop demo, a hand-rolled fake for a laptop test, or a wrapper around any other networking library you already use — the libraries just call the methods they need from whatever object you pass.  Your tests run on your laptop without hardware, and production swaps in the real socket without any code change.  If you've fully replaced the default, [Slimming Your Deploy](docs/contributing/slimming-your-deploy.md) shows how to drop `chumicro-sockets` from the on-device files too.
