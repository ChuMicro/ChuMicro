...
The first is the **asyncio module**: the event loop, the task scheduler, the stream layer.  MicroPython ships a capable one in its firmware.  CircuitPython's is a separate add-on library whose stream layer has imported a MicroPython-only module since 2021, so opening a connection through it fails on CircuitPython, and smaller boards don't include async support in the firmware at all.  A library built on asyncio quietly commits you to one runtime, and its event loop would fight the runner over who owns your main loop.

The second is the **`async`/`await` syntax**, and rejecting it was a separate, deliberate call.  The keywords compile down to the same generator machinery ChuMicro drives directly, so nothing is lost by skipping them, and four things get better ([Decision 0087](plans/decisions/0087-generators-for-sequential-io.md) carries the full case, in this order of weight):

- **A pause point can't be faked or forgotten.**  `yield from` only accepts something that actually suspends; put it in front of a plain function and Python raises `TypeError` on the spot.  With `await` the natural habit is to sprinkle the keyword on every helper, including pure-CPU ones, and the asyncio world needs linters to hunt the result after the fact.  On a board whose whole scheduling story is "every other service runs between your yields," an honest marker is worth more than the familiar idiom.
- **Every pause is visible.**  A `yield from` is one line in your source and one bytecode in the compiled form.  You can set a breakpoint on it, and when a deployed board misbehaves, the serial traceback shows which line it was waiting on.  `await` hides the same handoff behind compile-time machinery that differs per runtime.
