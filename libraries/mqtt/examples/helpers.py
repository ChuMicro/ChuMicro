"""Wifi-up helper for `chumicro-mqtt` examples.

Hides the runtime-specific wifi setup so each example stays focused
on the mqtt API.  Uses raw runtime primitives (CP `wifi.radio`, MP
`network.WLAN`) so the helper doesn't pull in any library that
`chumicro-mqtt` doesn't already depend on.
