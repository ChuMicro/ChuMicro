...
The pin numbers are MCU GPIO numbers, which `chumicro_compat.wiring` resolves into `machine.I2C` or `busio.I2C`; a transport takes any constructed bus of the matching runtime, so a hand-built one works too.

Construction blocks about 60 ms for the panel's power-on settle and mode forcing.

## Writing text

`write(text, row=..., column=...)` positions the cursor and sends the characters.  Text is clipped to the row rather than wrapped: the HD44780's native wrap lands mid-line on the other row and never reads as intended.  Positions outside the panel geometry raise `ValueError`, and so does a character above code point 255: the controller's character ROM is 8-bit, and on the common A00 ROM the upper half holds katakana rather than Latin-1 accents, so `"café"` renders a different glyph than the code point names.
...
The default is 16x2.  Row addresses derive from `columns`, since a four-row panel continues lines 0 and 1 into rows 2 and 3 after that many cells, so 16x4 and 20x4 panels address their lower rows correctly with the same class.  The controller addresses at most 40 columns and 4 rows; anything larger is refused at construction.
...
A transport is any object with `write_byte(value)` putting one raw byte on the PCF8574.  The shipped transports take an already-constructed bus, so the library imports no hardware modules; the CircuitPython one locks around every byte so display traffic interleaves politely with sensors on a shared bus, and it spins until the lock is granted, so it assumes no other code holds the bus across a write.  A custom backpack address (solder jumpers, or the 0x3F A-suffix parts) passes at construction:
...
The controller's busy flag is never read; timed waits are simpler, universal, and the datasheet numbers are generous.  `clear()` sleeps 2 ms, since the clear instruction needs 1.52 ms where every other one needs 37 us.  Each character costs four I2C writes (two nibbles, two enable states), so a full 16-character row is 68 one-byte transactions of about 200 us each on a 100 kHz bus: roughly 14 ms of wire time for a full-row rewrite, nearly three times the 5 ms tick budget a cooperative loop keeps.  Write a few cells per tick instead, about six at 100 kHz, or rewrite a row only on a change that arrives once a second or slower.  The PCF8574 is a Standard-mode 100 kHz part, so headroom comes from writing fewer cells rather than from a faster bus.  The `sleep_ms` constructor seam injects a recorder in tests so nothing actually waits.
...
The PCF8574 is rated for 100 kHz and `i2c_bus` defaults to 400 kHz on both runtimes, so pass `frequency=100_000`.  A hand-built bus has the same trap on one side: `machine.I2C` defaults to 400 kHz on both the rp2 and esp32 ports, while `busio.I2C` defaults to 100 kHz.  Backpacks vary on whether they populate the SDA and SCL pull-ups, and a board relying on the MCU's internal pull-ups will not clock reliably above 100 kHz; 4.7 kΩ to 3V3 on each line fixes it.  CircuitPython refuses outright, since `busio.I2C` tests for an external pull-up before it will construct.
