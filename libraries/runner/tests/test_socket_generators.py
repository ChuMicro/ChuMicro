...
the wrapper that ``Runner.add_generator`` wraps around it.  A
full-stack ``Runner.add_generator`` test at the end verifies the two
layers compose.

The wrapper's resume value (what ``handle()`` ``.send()``-s back into
the generator) is ``now_ms``; the helpers don't use it.  Tests
mirror that: ``gen.send(0)`` is the resume convention.
"""

import errno

from chumicro_runner import Runner
from chumicro_runner.generators import connect, recv_exact, recv_until, send_all
...
# -- Full-stack integration through Runner.add_generator -----------
