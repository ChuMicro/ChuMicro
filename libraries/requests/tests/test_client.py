"""HttpClient tests for chumicro_requests — GET/POST against FakeSocket.

HttpClient is runner-shaped (check / handle), single in-flight, single
socket per request.  Covers the runner contract, timeouts, busy-state
enforcement, oversize policy, redirects, response decoding, default
header merge, the FakeHttpClient fake, and ``from_config``.  The
wire-format surface (URL parse, header dict, request encoding, response
parser, redirect resolution) lives in the sibling ``test_wire.py``;
