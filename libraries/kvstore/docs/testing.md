...
<!-- GENERATION INSTRUCTIONS — delete this block once the page is written.
     ...
     consumers need to fake, delete all testing-helper references:
       ...
       2. Delete `src/chumicro_kvstore/testing.py`
     ...
     To fill this in, read:
       1. `src/chumicro_kvstore/testing.py` — the fake classes/functions
...
`chumicro_kvstore.testing` provides ...
...
```python
from chumicro_kvstore.testing import Fake...
...
Libraries that depend on `chumicro-kvstore` can import the fakes directly:
