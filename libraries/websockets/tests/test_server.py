...
# ---------------------------------------------------------------------------
# Fakes (also lifted to chumicro_websockets.testing in slice 4)
# ---------------------------------------------------------------------------
...
class FakeListener:
    ...
    ``queue_accept(socket)`` enqueues a peer socket that the next
    ``accept()`` call returns.  EAGAIN when the queue is empty.
    """
    ...
    def queue_accept(self, peer_socket: FakeSocket) -> None:
        self._pending.append(peer_socket)
