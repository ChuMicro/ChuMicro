...
``chumicro-dev-config.toml`` (top-level repo config — generated by
``python scripts/run.py setup`` from
``scripts/templates/chumicro-dev-config.toml.template``).  When
the shim isn't present (the steady-state for any clone without
local credentials) every test in this file early-returns cleanly
so the suite stays committable.
