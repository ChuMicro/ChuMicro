...
- Runner-tick budget: ≤5 ms per call in `tick()` / `handle()`; only `Runner.wait` blocks ([Decision 0080](plans/decisions/0080-runner-reactor.md)). No `async` / `await` / `asyncio` / `uasyncio` ([Decision 0087](plans/decisions/0087-generators-for-sequential-io.md), enforced by `CHU033` outside `functional_tests/`). No ISRs carrying application control flow; a device library may own a capture interrupt under the four conditions in [Decision 0124](plans/decisions/0124-buttons-and-knobs-libraries.md) (handler allocates nothing, captures raw edges only, no user callback in interrupt context, bounded and counted overflow). No blocking TCP/TLS handshake.  Use `chumicro_sockets`'s tick-driven connector ([0081](plans/decisions/0081-non-blocking-connect-via-tick-driven-connector.md)). Offenders: `time.sleep(>0.005)`, `select.poll(>0)`, blocking `getaddrinfo` / `socket.recv()` / `machine.lightsleep`, MP `os.urandom`.
...
- Don't write files to `/Volumes/CIRCUITPY` by hand either, `cp -r` included.  The board auto-reloads the moment the drive changes, so it is rewriting the filesystem while the host still is, and on macOS `cp` adds `._` AppleDouble files to the same directories.  That combination tears FAT directory entries: the host can list them, `stat` returns EINVAL, and they cannot be unlinked, which fails every later deploy until `chumicro-workspace reset-board --yes` reformats.  Deploy through `test-libraries-functional` or `chumicro-workspace deploy`, which quiesce the board first.
...
- A demo's `app.py` imports only what it demonstrates, prints its own `NAME key=value` marker lines, and lingers a few seconds before exiting ([Decision 0123](plans/decisions/0123-demos-read-like-a-first-project.md)). No `chumicro_test_harness` import in board-side demo code; `driver.py` still reaches anywhere.
- Demos and examples write the main loop out: `while ...:` then `runner.tick()` then `runner.wait(now_ms)` ([Decision 0122](plans/decisions/0122-demos-and-examples-write-the-loop-out.md)). `run_until` is fine in application code but hides the loop from the reader teaching code exists for. A `while True: runner.tick()` with no `wait(now_ms)` is the actual busy-spin to reject.
