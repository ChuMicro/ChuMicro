...
- Always pass preflight before commit. If preflight is already red on `main` (not from your changes), surface and stop. **No carve-out for doc-only, plans-only, handoff, or workstream-log commits** — CHU lints catch drift in plans-docs that `git diff` does not surface, and the "too small to gate" framing is the failure mode.
- Always invoke the [`task-checkpoint`](.github/skills/task-checkpoint/SKILL.md) skill at end of work. It owns preflight, `plans/next-up.md` refresh, durable-lesson lift, commit, and push (the skill's own rule: commit if the work is a coherent unit, leave uncommitted with a note if partial). **"End of work" is per coherent unit of work (one commit subject), not per session** — multiple checkpoints per session is normal; skipping the skill for "small" commits is the failure mode.
...
- **Sub-agent reports and session handoffs describe intent, not state.** Never build on a concrete claim from a sub-agent (Explore / audit-* / general-purpose) or a `plans/handoffs/` file without re-deriving from code, tests, or commands — *especially* one marked `[VERIFIED]`. The writer was near a context limit; the doc is an index, not a boundary.
- **Read the artifact, not the summary.** Never trust an exit code, "preflight passed" string, or sub-agent summary alone — re-read the file or re-run the smallest failing check. Always re-assert `pwd` after any external change; a "modified externally" notice means re-read the file before editing it again.
...
- While the repo is private, always commit directly to `main`. No feature branches, no PRs.
...
- **Git safety: never bypass safety without explicit ask.** No `git config` edits, no `--no-verify` / `--no-gpg-sign`, no force-push to main, no destructive ops (`reset --hard`, `checkout .`, `restore .`, `clean -f`, `branch -D`). Never `--amend` — a failed pre-commit means the commit didn't happen, so `--amend` would modify the *previous* one; always create a new commit. Never commit unsolicited.
...
- No backwards-compatibility burden until 1.0. Never deprecate or provide legacy support.
...
- Always skim [`plans/patterns.md`](plans/patterns.md) for an established shape before writing implementation code in `libraries/` or `workbench/`.
- Always invoke the [`new-library`](.github/skills/new-library/SKILL.md) skill before scaffolding a new library. It owns directory shape, `__init__.py` + `testing.py` layout, test placement (`tests/` cross-runtime by default; pytest-fixture suites opt down via `__chumicro_runtimes__ = ("cpython",)`), initial `VERSION` seed, `pyproject.toml`, and `__all__`. `python scripts/run.py new-library --workbench` scaffolds a host-only tool. Reading an existing library to copy its shape is a fallback only.
...
- **A `plans/workstreams/<name>.md` file you're picking up is a directive, not a research artifact.** Execute the next unshipped phase from its `## Implementation phases` / `## Plan` section. Verify any claim that looks changed (read the code, run the test, check the commit), then proceed. Never bounce to the user without a concrete blocker — pausing without one is the failure mode. As each phase lands, append one line to `## Validation history`; update the `Status:` line (`shipped`/`parked`/`superseded`) when fully done.
- Always run `--help` on `python scripts/run.py`, `chumicro-workspace`, `chumicro-deploy`, or `chumicro-repl` (and each subcommand) before asking the user or giving up on a flag. The full subcommand inventory is discoverable. Treating a workbench tool or `scripts/run.py` as opaque is a process failure.
- **Always deploy through `chumicro-workspace deploy`** (or `chumicro-deploy` for raw file trees the project shape doesn't support). Both run the one device-staging path: diff + `rsync --delete`. Never bypass.
- **A library public-API or behavior change requires a `VERSION` bump in the same commit** (once per coherent unit of work — a full audit pass is one bump, not one per commit). `python scripts/run.py check-api` confirms the minimum level (patch / minor / major). Behavior-only, docstring-only, and shipped-comment-only changes still bump `patch` so users see the change land in their version pin. Detail in [docs/contributing/releases.md](docs/contributing/releases.md).
...
- A **cross-runtime test file** is the default for `tests/test_*.py` — loads on CPython + MP/CP unix-ports + real silicon via `chumicro_test_harness`. **Never `import pytest`**; use plain `assert` and constructor-injected fakes from each library's `testing.py`. Files opt down via in-file markers: `__chumicro_runtimes__ = ("cpython",)` for pytest-fixture suites (free to `import pytest`, use `monkeypatch`); `__chumicro_host_only__ = True` for host-only (next bullet). The `_pytest.py` filename suffix is a human hint, not load-bearing — the marker is the contract. The file's top docstring must always say which lane it's on.
...
- When writing tests, depend only on the package's own `src/` + `testing.py`, stdlib, pytest + plugins, and `chumicro_test_harness` (including its submodules — see [Decision 0082](plans/decisions/0082-test-harness-as-infrastructure-library.md)). Never `import chumicro_<other-package>` for inputs and never read sibling-repo filesystems.
- **Real-board bake catches orchestration bugs unit tests miss.** Changes touching host-side concurrency, streaming-transport plumbing, marker dispatch, multi-call sequences against the same transport, or abort / cleanup paths need a real-board bake before "done", *even when the unit suite is green*. Three live examples in one session: the `pytest_collection_modifyitems` deselect sweep dropped host-only items (unit tests only exercised `pytest_collect_file`); `bind_to` consumed `SERVER_READY` once and hung on the second `hit` (single-call tests didn't surface it); the MP transport returned empty stdout when `data_consumer` was wired (the test fake returned the convenient shape regardless). All three were green under unit tests and surfaced the first time a real-board run exercised the full sequence. Run a representative demo or Category 1 functional test against a real board before reporting orchestration-shaped work complete.
- **A test stand-in fake must model the target's awkward shapes, not just the easy path.** A fake that returns the convenient result regardless of inputs hides production bugs that only surface against the real component. Concrete instance: `FakeSerialTransport.exec_raw` originally returned the full `(stdout_bytes, stderr_bytes)` tuple regardless of whether `data_consumer` was passed, while real mpremote returns `(b"", stderr_bytes)` when a consumer is wired (because its `read_until` replaces its internal `data` per-byte instead of appending). Production was broken in the streaming path; the unit suite passed because the fake was lying. If the target branches on a parameter, the fake reproduces both branches; if the target returns different shapes per code path, the fake matches.

**Code shape (libraries, microcontroller only)**

- **Runner-tick budget: ≤5 ms per library call reachable from `tick()` / `handle()`.** Only `Runner.wait` blocks ([Decision 0080](plans/decisions/0080-runner-reactor.md)). No `async` / `await`, no ISRs, no synchronous TCP/TLS handshake from runner-tick paths — use `chumicro_sockets`'s tick-driven connector ([Decisions 0014](plans/decisions/0014-runner-pattern.md), [0081](plans/decisions/0081-non-blocking-connect-via-tick-driven-connector.md)). Named offenders: `time.sleep(>0.005)`, `select.poll(>0)`, blocking `getaddrinfo` / `socket.recv()` / `machine.lightsleep` / MP `os.urandom`. The budget is the test, not the list.
...
- **`gc.collect()`: forbidden in hot paths, required after substantial import-time state, allowed between state changes.** Hot-path use signals missing pre-allocation — fix that. Pattern: `import gc as _gc; _gc.collect(); del _gc` at end of any library `__init__.py` whose top-level imports build substantial state (parsers / decoders, multi-class setups, runtime-specific adapter loads, module-scope `const()` tables); tiny single-purpose libraries don't earn it. Allowed post-handshake / post-bulk-decode / post-`connect()`. Benign on CPython. Rationale + measured evidence: [Decision 0084](plans/decisions/0084-gc-collect-policy.md).
...
- **Changes touching I/O, hot paths, or runtime-specific behavior require real-board verification before "done."** Unit-test green is necessary but not sufficient: deploy to the hardware tier the library targets (Pico W at minimum for the 256 KB cross-runtime contract), run a one-minute REPL tail under representative load, confirm no tracebacks / safe-mode banners / silent stalls. `chumicro-workspace deploy-example` or `chumicro-workspace deploy [<project>]` + `chumicro-workspace tail` is the path. Audit passes touching hot paths default to this check; ADR-driven changes name the validation in the commit message.
- Always use constructor injection for time, I/O, and network dependencies. Fakes go in the library's `testing.py` submodule.
- **Constructor-injected substrate stays runtime-optional.** Declare the substrate (`chumicro-sockets`, `chumicro-timing`) in `pyproject.toml` `dependencies` but keep `src/<name>/` free of top-level `import chumicro_<substrate>` — keeps it off the deploy bundle and out of RAM for BYOS consumers. Duplicated trivial helpers per consumer (`_is_eagain` / `_force_non_blocking` in each networking lib) and lazy in-function imports hold this up; hoisting helpers into the substrate is the trap. Grep pre-edit `src/` for top-level `chumicro_<X>` imports before adding one — zero + declared dep means intentional optionality.
...
- **A test docstring describes what the asserts actually check, not what the test name aspires to.** Causal connectors (`so`, `because`, `therefore`) require real causation observable in the test body; independent checks join with `and` or a comma. A test named `test_sleep_ms_advances_the_clock` whose only asserts are `elapsed >= 0` and `elapsed < 500` does not prove advance (zero is allowed) — the docstring must say what the asserts prove, not what the name aspires to.
...
- Two CP boards plugged in mount as `/Volumes/CIRCUITPY` + `/Volumes/CIRCUITPY 1` naturally — that's normal disambiguation, NOT a wedge. Run `chumicro-workspace devices` first; only treat as wedge if one CP board is listed but the mount layout is anomalous. Parallel deploys to two CP boards race for the mount; run them sequentially.
...
- **Grep is for verifying a specific claim, not for replacing the read on judgment tasks.** When the deliverable is a judgment (audit, code review, comprehending unfamiliar code), workload size is not permission to drop to pattern-scanning. Split the target into smaller sequential passes; do not switch tools. Token cost is not the success metric when the work *is* the read. Grep is the right primary tool when the deliverable is *finding* something specific (callers of X, occurrences of a pattern); it is the wrong primary tool when the deliverable is *judging* prose, structure, or design.
- **An error message is a symptom, not a root cause — capture the next layer of detail before naming the cause.** A wrapped exception printed as `type(error).__name__: error` strips the traceback; a host-side TypeError can be observing a board-side crash; a USB-CDC drop can itself be the downstream effect of a hard fault.  Naming a cause from the symptom looks productive and is easy to "verify" with a few targeted greps, but the verification is confirmation bias dressed up as code reading.  The fix is always cheap — re-run the smallest failing check with maximal detail (`traceback.print_exc()`, board stdout tail, kernel log) before theorizing.  Worked instance from this session: `argument must be an int, or have a fileno() method` was first named as a `chumicro_sockets` / `chumicro_runner` poll-registration bug; the captured traceback showed it was host-side `pyserial.in_waiting` failing on a stale fd; the stale fd was then named "USB-CDC drop from current draw" — both layers were symptom-as-cause, the actual bug was a chumicro-stack crash on the board.
...
- **Subagents default to `model: "opus"`.** When invoking the Agent tool for research, audit, or judgment tasks (Explore, general-purpose, audit-*), pass `model: "opus"` explicitly — the harness defaults subagents to Sonnet, but quality of research and judgment dominates token cost in this repo. Sonnet is fine for quick targeted lookups where speed beats depth.
...
Shapes that usually fail the read-aloud test:
...
- **Article tics.** `the` before brand names is usually wrong (`Pi Pico W`, not `the Pi Pico W`). `X is the one that Y` is wordier than `X does Y`. `the X of the Y of the Z` chains usually have one too many. Two nouns in one sentence often need different articles — `the` is not the default. Apply per-noun in every sentence of a rewrite; inherited `the`s compound.
- **Abstract opener + em-dash + concrete restatement is throat-clearing.** *"The config is declarative — list your devices in YAML"* should be *"List your devices in `devices.yml`."* Ask whether the pre-em-dash clause survives deletion — it usually should.
...
| `preflight --coverage-threshold 94` | Full CI mirror (lint + build + docs + all-runtime unit tests + checks). `--with-functional` adds hardware-gated tests; `--with-device-unit` appends the on-device sweep |
