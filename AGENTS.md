...
- **Never `git --amend`.** A failed pre-commit means the commit didn't happen — always create a new commit.
- **Never commit unsolicited.** Wait for the user's ask.
...
- **Always deploy through `chumicro-workspace deploy`** (or `chumicro-deploy` for raw file trees the project structure doesn't support). Both run the one device-staging path: diff + `rsync --delete`. Never bypass.
...
- **A test stand-in fake must model the target's awkward shapes, not just the easy path.** A fake that returns the convenient result regardless of inputs hides production bugs that only surface against the real component. If the target branches on a parameter, the fake reproduces both branches; if the target returns different shapes per code path, the fake matches.
...
Per-library inventory + dependency graph: [`libraries/README.md`](libraries/README.md). Workbench tools: [`workbench/README.md`](workbench/README.md).
