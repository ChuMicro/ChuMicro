...
- Always pass preflight before commit. If preflight is already red on `main` (not from your changes), surface and stop.
- Always invoke the [`task-checkpoint`](.github/skills/task-checkpoint/SKILL.md) skill at end of work. It owns preflight, `plans/next-up.md` refresh, durable-lesson lift, commit, and push (the skill's own rule: commit if the work is a coherent unit, leave uncommitted with a note if partial).
