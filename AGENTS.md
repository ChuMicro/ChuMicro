...
- **Absolute imports only in code that runs on devices.** `libraries/*/src/`, `support/test_harness/`, and `support/abstractions/` must use `from chumicro_foo.bar import baz`, never `from .bar import baz`. Relative imports break CircuitPython RAM-mode deploys — library modules are `exec()`'d without a `__package__`, so a leading `.` has nothing to resolve against. Workbench packages (`workbench/*/src/`) and tests may use either style. See the style guide's "Imports" section for the full explanation.
