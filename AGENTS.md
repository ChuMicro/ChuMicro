...
- Never `pkill` / kill a device run mid-deploy. `mpremote fs cp` is additive (no `--delete`), and the sweep tracks which trees it staged only in *in-process memory* — a killed run orphans `chumicro_*` trees on LittleFS that no later run knows to delete, so additive copies pile up to `ENOSPC` and wedge the board. The sweep also does not defensively wipe before its *first* stage, so pre-existing board residue (from any prior run/session) cascades the same way. Let a slow run finish, or recover with `chumicro-workspace reset-board --device <id> --yes` then re-run a single library cleanly. Isolated-pass / sweep-fail of unrelated tests is the signature of this cumulative-state fragility, not a code bug.
