...
- All time math goes through `chumicro_timing` (`ticks_ms`, `ticks_diff`, `ticks_add`, `Heartbeat`) in `libraries/` source and in any tests gating on deadlines or elapsed time. Never call `time.monotonic`, `time.ticks_ms`, or `supervisor.ticks_ms` directly — mixing them pits a `chumicro_timing`-tick deadline against a wall clock and produces a "freeze" that's really mismatched units. Exception: `chumicro_timing`'s own resolution tests.
- **Steady-state zero allocation per tick.** Inside `tick()` / `handle()` / `check()` / any parser inner loop, allocation delta over 1000 ticks ≤ 64 bytes — verify with `gc.mem_alloc()` bracketed by `gc.disable()` ([audit-embedded §10](.github/skills/audit-embedded/SKILL.md)). Common offenders: f-strings / `.format()` (use `log.info("...%d...", n)`), dict/list/tuple/set literals (reuse scratch), `bytes(view[a:b])` to feed `struct.unpack` (use `unpack_from`), `enumerate` / `zip` / `reversed` in a loop, `self.x.y.z.method()` chains used twice or more (cache to a local). Full offender catalogue with rewrites in [audit-embedded §3](.github/skills/audit-embedded/SKILL.md).
- **`gc.collect()`: forbidden in hot paths, required at `__init__.py` end, allowed between state changes.** Hot-path `gc.collect()` signals missing pre-allocation — fix that. Required `import gc as _gc; _gc.collect(); del _gc` at every library `__init__.py` end (and between submodule imports for multi-file libraries) — MicroPython's compile-time AST scratch otherwise interleaves with persistent state (+33 KB contiguous heap measured on Pi Pico W MP, enough to flip TLS handshakes from ENOMEM to ROUND_TRIP). Allowed post-handshake / post-bulk-decode / post-`connect()` where pre-emptive collection beats triggering inside the next allocation. Benign on CPython.
