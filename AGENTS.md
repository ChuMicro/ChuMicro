...
- **`gc.collect()`: forbidden in hot paths, required after substantial import-time state, allowed between state changes.** Hot-path use signals missing pre-allocation — fix that. Required pattern: `import gc as _gc; _gc.collect(); del _gc` at the end of any library `__init__.py` whose top-level imports build substantial state (parsers / decoders, multi-class setups, runtime-specific adapter loads, module-scope `const()` tables); tiny single-purpose libraries don't earn it. Allowed post-handshake / post-bulk-decode / post-`connect()`. Benign on CPython. Rationale + measured evidence: [Decision 0084](plans/decisions/0084-gc-collect-policy.md).
