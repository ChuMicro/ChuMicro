...
- **No mono-repo references in publishable `src/` trees** — `libraries/*/src/`, `workbench/*/src/`, and `support/*/src/` ship to PyPI / CircuitPython-bundle consumers who don't have the mono-repo checked out.  Those trees must not name `Decision NNNN`, point at `plans/...md` paths, mention `scripts/run.py` (the mono-repo's developer command runner — distinct from a workspace's own `run.py` shim), or use "chumicro mono-repo" framing.  Inline a one-line summary instead of cross-linking; rationale and history belong in `plans/` and `docs/`.  Enforced by `CHU006` in [`scripts/check_no_repo_refs.py`](scripts/check_no_repo_refs.py).  Suppress with `# noqa: CHU006` only when the reference is genuinely the only useful pointer (rare).
