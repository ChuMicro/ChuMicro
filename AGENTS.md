...
- **Pick the right workbench tool, not the first one whose name matches.** `chumicro-workspace deploy` is the front door for any project-shaped deploy — it knows `devices.yml`, runtime-config msgpack, the `app.py` + `run()` boot-shim, and `--import-graph` (AST-walks the entrypoint, ships only transitively-imported modules). For a project with `app.py`, the right call is `chumicro-workspace deploy [<name>]` — boot-shim and import-graph auto-detect. `chumicro-deploy deploy --directory` ships the *whole tree* including unused libraries, wastes flash, and can OOM at boot; reach for it only for non-project deploys. Before picking a flag, check `workbench/<tool>/examples/` for a worked invocation — `import_graph_deploy.py`, `file_map_deploy.py`, etc. — and the tool's `--help`. Wrong-tool, wrong-flag deploys are a recurring failure mode the `--help` rule above didn't fully cover.
...
- **No allocation per tick.** Inside `tick()` / `handle()` / `check()` / any parser inner loop: no f-strings or `.format()` (each allocates a new `str` — use `log.info("...%d...", n)` with logger-side interpolation, or guard with an `is_enabled` check); no dict / list / set literals (reuse a module-level constant or a cleared scratch container); `bytes(view[a:b])` to feed `struct.unpack` → use `struct.unpack_from(fmt, buf, offset)`; `bytes(view).decode("utf-8")` → use `str(view, "utf-8")` (the 3-arg constructor accepts buffer-protocol objects, skips the copy); `int.from_bytes(bytes(buffer[a:b]), ...)` — drop the `bytes()` wrapper, `int.from_bytes` accepts any buffer-protocol object directly; `self.x.y.z.method()` chains used twice or more in a loop → cache to a local once before the loop. F-strings are fine outside hot paths.
