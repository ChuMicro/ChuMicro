...
- **No mono-repo references in publishable `src/` trees** — `libraries/*/src/`, `workbench/*/src/`, and `support/*/src/` ship to PyPI / CircuitPython-bundle consumers who don't have the mono-repo checked out.  Those trees must not name `Decision NNNN`, point at `plans/...md` paths, mention `scripts/run.py` (the mono-repo's developer command runner), name a bare `run.py` (the workspace-template's shim — only `chumicro_workspace` legitimately knows about it; everywhere else, name the installable CLI like `chumicro-deploy` instead), or use "chumicro mono-repo" framing.  Inline a one-line summary instead of cross-linking; rationale and history belong in `plans/` and `docs/`.  Enforced by `CHU006` in [`scripts/check_no_repo_refs.py`](scripts/check_no_repo_refs.py).  Suppress with `# noqa: CHU006` only when the reference is genuinely the only useful pointer (rare).
...
- **Plans-doc brevity — `next-up.md` and `now.md` are agent-managed** — every top-level bullet in [`plans/next-up.md`](plans/next-up.md) and [`plans/now.md`](plans/now.md) is capped at 5 bullet markers (lead + sub-bullets together).  Anything bigger gets promoted to a workstream file under `plans/workstreams/` (open) or `plans/workstreams/archive/` (shipped) and replaced here by a one-line pointer.  `now.md` is additionally capped at 25 total lines — it is the 30-second snapshot, overwritten each `task-checkpoint`, not a place for durable rules.  When updating one file, cross-maintain the other (the snapshot in `now.md` mirrors `## Now` in `next-up.md`).  Enforced by `CHU011` in [`scripts/check_plans_brevity.py`](scripts/check_plans_brevity.py).  Suppress with `<!-- noqa: CHU011 -->` (sparingly).
