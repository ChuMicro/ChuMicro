...
- `main` is the only long-lived branch ([Decision 0019](plans/decisions/0019-branching-model.md)). Commit maintainer and agent work directly to `main`. No feature branches. Outside contributors fork and open a PR against `main` ([CONTRIBUTING.md](CONTRIBUTING.md)); that flow is what contributor-facing docs describe, not what you follow. The one other branch 0019 allows is a short-lived `release/<library>-vX.Y.x` cut from a stable tag to hotfix an older release, deleted once the patch ships.
