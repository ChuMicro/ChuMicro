...
- **`gc.collect()`: forbidden in hot paths, required at `__init__.py` end, allowed between state changes.** Hot-path `gc.collect()` signals missing pre-allocation — fix that. Required `import gc as _gc; _gc.collect(); del _gc` at every library `__init__.py` end (and between submodule imports for multi-file libraries). MicroPython's compiler leaves AST scratch + transient tuples interleaved with the library's persistent state, and auto-GC may not fire to reclaim it — measured +33 KB contiguous heap on Pi Pico W MP, enough to flip TLS handshakes from ENOMEM to ROUND_TRIP. Allowed post-handshake / post-bulk-decode / post-`connect()` where pre-emptive collection beats triggering inside the next allocation. Benign on CPython.
