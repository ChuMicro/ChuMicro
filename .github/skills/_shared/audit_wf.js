...
const [loader, cold, craft, orchestration, surprise, ideas, research] = await parallel([
  ...
  () => agent(
    ...
    `2. THE QUESTION RULE — any step that asks the user to decide places the finding's evidence, consequence, and exact proposed change in front of them in the same message; a question answerable only by trusting the asker is IMPORTANT (cryptic chip-label approval prompts are the recognizer). Ask text the skill *prescribes* — a literal AskUserQuestion question, option label, or description written into a step or example — clears the wording floor in walk-pattern.md, the "Wording the ask" section: the question stands alone, each option description is a full sentence that does not restate its label, and no em-dash, " -- ", arrow, or middot glues an instruction outside a quoted span. A prescribed option label trailing its instruction after an em-dash, or gate text in lens-shorthand a cold user could not decode, is a finding even though it parses — ask_gate.py bounces it only once the ask fires, so this lane is the earlier catch.\n` +
