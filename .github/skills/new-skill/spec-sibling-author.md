...
Reading-rule, reference-file-layout, and MCP-tool-naming rules for skill authors.  Sibling trigger overlap is measured by the probe lane (`trigger-evals.json` near-misses against the live registry), not judged by a reader; the rules here govern what the author writes.
...
- [`interview.md`](interview.md) and [`template.md`](template.md) inside the new-skill directory
- The user's answers during the interview

**Narrow exception:** the sibling survey (Process step 1 in the new-skill SKILL.md) reads ONLY the `description:` line of each sibling — never the body, never the persona files, never the reference files.  Description-line scan is for trigger-routing comparison; nothing else.
...
| `spec.md` (+ `spec-*.md`) | Rules / criteria the skill enforces — for skills whose job is to evaluate other artifacts against a standard. Split by topic when a single spec file grows past the splitting threshold. |
