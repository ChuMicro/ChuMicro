...
Lens findings outrank your own read: every lens finding enters the report unfiltered, at the lens's tier — disagree in prose next to the number, never by dropping or re-tiering. Print one numbered report — findings first (tier, quoted evidence, consequence, exact proposed fix), then the ideas menu with each lens-recommended action. The user picks in plain chat: `apply 1, 3` · `discuss 2` · `edit 4: <wording>` · `skip the rest`; when the open items fit in 4 or fewer, offer the pick as a `multiSelect` AskUserQuestion instead (one option per number) — past that, plain chat avoids the tool's 4-option cap. Past ~8 open items, render and serve the shared decision page (`.github/skills/_shared/picker/render_picker.py` + `serve_picker.py`, background, Monitor on its stdout) so picks and per-item notes come back as one submitted blob; typed picks stay valid throughout.
