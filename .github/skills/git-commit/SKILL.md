...
Follow the project's commit-message conventions: imperative subject line, body explaining *why*.  No `Co-Authored-By: Claude …` trailer — this repository's convention is that commits are authored by the human running the agent.  A local `commit-msg` hook (`.git/hooks/commit-msg`, not tracked) strips any that slip through, so don't be surprised if a trailer you typed disappears in `git log`.
