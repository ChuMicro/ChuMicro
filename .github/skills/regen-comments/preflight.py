...
Every grounding/writing/verify layer runs as a `claude -p` subprocess, which uses the `claude` CLI's OWN
authentication — not the account of the session you launched the skill from. If you are signed into the
Claude app/IDE as one account but the `claude` CLI on PATH is logged in as another (different org, model
access, subscription, billing), the clean-room work silently runs under the wrong account. This reports the
CLI's logged-in account (`claude auth status`) so a mismatch is caught up front, and `--expect-email` flags
it outright when the orchestrator passes the session's account.
