...
function ledgerPrompt(codeResults, commentLedger, domain, feedback) {
  ...
  return 'You are the ledger-writer. Merge findings from code-reading lenses and a comment-triage lens over '
    ...
    + '- Plain words, no coinage: do not invent a hyphenated compound LABEL for a thing (a writer pastes it). '
    + 'State the behavior plainly and let the writer name it -- write `holds items, drops the oldest when '
    + 'full`, not `drop-oldest-when-full buffer`.\n'
    + '- CULL HARD to what the writer cannot get itself. The docstring writer READS the code it documents, '
    + 'so DROP, even when true and grounded: (a) a guard, validation, or default visible in the symbol '
    + 'itself; (b) a plainly-positive single-method behavior; (c) TRACING or POSITIONAL facts with no caller '
    + 'consequence -- the internal data flow of a value inside a symbol, which line assigns or reads it, or a '
    + 'step plainly visible in the body. The writer recovers all of these from the code, and feeding them only '
    + 'inflates the docstring.\n'
    + '- KEEP only a fact that changes what a CALLER does or expects: (a) CROSS-METHOD facts (a value\'s '
    + 'referent or role across methods, a relationship spanning symbols); (b) NON-DERIVABLE contracts or '
    + 'domain facts; (c) ABSENCE / negative facts -- what the code does NOT do (a handle never closed, a '
    + 'buffer not size-checked, a value not persisted across a restart, an error swallowed rather than '
    + 'raised). A writer reads what the code DOES, never what it '
    + 'omits, so only the ledger can carry an absence. If a fact is internal bookkeeping with no caller '
    + 'consequence, DROP it.\n'
