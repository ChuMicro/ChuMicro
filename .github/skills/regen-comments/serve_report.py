#!/usr/bin/env python3
"""Serve a run room's compare page over localhost and loop its Submit button back to the session.

compare.html carries a Copy-selection bar whose blob the human pastes back into the session. Served
over http, the same bar also shows a **Submit to session** button: it POSTs the selection blob here,
and this server writes it to <dir>/selection.txt and prints one `SELECTION RECEIVED -> <path>` line
...
Usage: serve_report.py <dir> [<page>]      (default page: compare.html)
...
def main():
    ...
    page = sys.argv[2] if len(sys.argv) > 2 else "compare.html"
