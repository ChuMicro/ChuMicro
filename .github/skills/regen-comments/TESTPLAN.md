...
| A12c | `regen_phase2.py <room> plain --tight` on a room with a kept ledger | every docstring ≤ 2 sentences, zero Args/Returns/Raises sections, self-documenting symbols bare; every ledger fact present (summary or inline `#`); `CODE IDENTICAL`; `<room>/phase2.json` records `{"voice": "plain", "genre": ..., "tight": true}` |
...
| B8 | **refine keeps the run shape** | on a finished `--tight` room, `regen_symbol.py $RUN plain <sym>` → that symbol is STILL ≤ 2 sentences with no sections (phase2.json honored — the old bug regenerated it in default code shape); same check on a `--kind test` room (stays a one-sentence claim); `CODE IDENTICAL` |
...
| C10 | `/regen-comments <a source file> --tight` | summary-only docstrings (1–2 sentences, no Args/Returns/body) on every non-bare symbol; in the refine loop, exhaust the 4 cached takes on one symbol so `regen_symbol.py` fires → the fresh take is **still tight** |
