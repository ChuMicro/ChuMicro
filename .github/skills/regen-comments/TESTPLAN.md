...
| A12c | `regen_phase2.py <room> plain --tight` on a room with a kept ledger | PEP 257 shape: single sentence summary lines, bodies tiny (≤ 2 sentences) and rare, **zero** Args/Returns/Raises sections, self-documenting symbols bare; ledger facts present as clean sentences or deliberately dropped (never crammed); `CODE IDENTICAL`; `<room>/phase2.json` records `{"voice": "plain", "genre": ..., "tight": true, "less": false}` |
| A12d | `regen_phase2.py <room> plain --less` on a room with a kept ledger | PEP 257 shape: **single sentence summary lines** WITH short Args/Returns/Raises where they apply; bodies are **tiny (≤ 2 sentences) and rare (a few spots, never most symbols)**; no entry padded into prose; `phase2.json` records `"less": true`; passing `--tight --less` together is **refused**; `CODE IDENTICAL` |
...
| C10 | `/regen-comments <a source file> --tight` | PEP 257 minimal shape (single sentence summaries, rare tiny bodies, no sections) on every non-bare symbol; in the refine loop, exhaust the 4 cached takes on one symbol so `regen_symbol.py` fires → the fresh take is **still tight**. Repeat with `--less` → identical shape plus short Args/Returns/Raises sections (the only difference) |
