...
| A8 | fabricate a run room (target + `runs/run-1..4.py` + `pick.json` + flags JSONs + `FINAL_plain.py`), `render_report.py` it | every card open on load: the original docstring on top, one candidate box per **differing** pass beside the pre-checked suggested one (comparison chips; 4+ boxes scroll horizontally), a seeded edit box below, sticky Copy-selection bar; extracted `<script>` passes `node --check` |
| A9 | `apply_selection.py parse` a shared PICKS blob (`sym = run-N`, `sym = edit` + an `edit <id>:` line + a `note <id>:` line) and a legacy blob (fenced `#edit` block, `#note` lines) | JSON carries picks/edits/notes exactly — the edit line decodes `\n` / backslash escapes and the note stays a note (an edit pick with no edit line takes the note as its text: pre-edit-channel blobs); a `voice` blob parses with `kind: voice` and is **refused** by `apply` |
| A10 | `apply_selection.py apply` that blob, then `verify_code.py <orig> FINAL_plain.py` | splices land (incl. inline comments), a **class** pick takes the class docstring only (methods untouched), multi-line edit re-indented under the symbol with backslashes surviving verbatim, header preserved, `CODE IDENTICAL`; an all-suggested blob is a stated no-op |
