...
| A12c | `regen_phase2.py <room> plain --tight` on a room with a kept ledger | every docstring ≤ 2 sentences, zero Args/Returns/Raises sections, self-documenting symbols bare; every ledger fact present (summary or inline `#`); `CODE IDENTICAL` |
