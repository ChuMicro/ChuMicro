...
7. **Push back on lossy, risky, or out-of-directive requests — do not blindly comply.** When the human asks for something that would lose data (drop a fact they kept, a blunt body-drop), is risky (apply without reading the report, anything that touches code, committing), or falls outside what the skill is built to do, name the consequence and offer options via `AskUserQuestion` instead of just doing it — e.g. *"that drops the by-reference fact you kept: **tighten instead** (keeps it) / **drop anyway** / **cancel** / **re-roll with a new rule added to the ledger**."* Comply only on explicit confirmation. This applies everywhere, not only in the refine loop.
...
- **edit the facts** — *drop / edit*: change `ledger_final.md`, then `regen_symbol.py <symbol>` (re-runs the writer against the edited ledger and splices only that symbol back; others stay). *add*: a **correction** goes through `stubify_fact.py` (validates: confirmed → append stub + regen; contradicted → raise suspicion, never block; unverifiable+checkable → ask for a source; else trust); a **your-own-note** is kept verbatim as `# NOTE(<initials>): …` at the symbol (ask initials; offer voiced vs verbatim) — the comment lens re-harvests it later.
