...
function tightPrompt(n) {
  ...
  return voiceBlock
    ...
    + (LESS
        ? 'Each class, function, and method gets a docstring built like this: line one is a single '
          + 'sentence ' + register + ' carrying purpose and caller contract; after a blank line, a tiny '
          + 'body of a sentence or two may follow in the few spots a fact earns it; then short Args and '
          + 'Returns (or Raises) sections where they apply, each entry one short sentence. '
        : 'Each class, function, and method gets a docstring of one sentence ' + register + ' carrying '
          + 'purpose and caller contract, at most two when the contract earns it. Nothing follows the '
          + 'summary: no body paragraph, no Args / Returns / Raises sections. '
      )
    + 'A symbol with no ledger fact and a name that says everything gets no docstring. Modules get one '
    + 'to three sentences on what the file does. The docstring covers BEHAVIOR and the caller contract '
    + 'only; traps and line mechanics ride as `#` sentences above their lines, in the voice. Each '
    + 'ledger fact lands once, and a fact that will not fit as a clean sentence is dropped, never '
    + 'crammed. Weave a warning into the sentence that states it, never behind scaffolding like "one '
    + 'thing to know".\n\n'
...
function voicedPrompt(n) {
  return VOICE_PARA + ' Every docstring and comment is fully in that voice.\n\n' + sampleBlock()
    ...
    + 'above its line, in the same voice. Each ledger fact lands exactly once; drop none. Weave a '
    ...
    + 'Build each docstring like this: line one is a single sentence summary in the same voice; after a '
    + 'blank line, a body of at most '
    + 'three or four sentences may follow where the symbol earns one. Every sentence is '
...
function genPrompt(n) {
  ...
  return 'Read the code at ' + FILE + ' and the nuance ledger at ' + LEDGER + ' together. The ledger holds the '
    ...
    + 'smooth, flowing sentences. Build each docstring like this: line one is a single sentence summary; '
    + 'after a blank line, a body of at most '
    + 'three or four sentences may follow where the symbol earns one.\n\n'
    ...
    + 'the docstrings, never a compressed note: never echo the ledger\'s notation, never splice fragments '
    + 'with colons, never use `->` arrows.\n\n'
...
function selectPrompt() {
  ...
  if ((TIGHT || LESS) && GENRE !== 'test' && GENRE !== 'functional_test' && GENRE !== 'example') {
    job = 'YOUR JOB (' + (LESS ? 'LESS' : 'TIGHT') + ' MODE): pick the file whose '
      + (LESS ? 'single sentence summary lines' : 'one-or-two-sentence docstrings')
      + ' are the sharpest and most exact, in the voice throughout -- a flat summary '
      + 'loses to an alive one saying the same thing. REJECT a file that '
      + 'splices clauses with " -- ", leaves a symbol bare that still carries a ledger fact, narrates '
      + 'traps or mechanics in a docstring instead of a `#` sentence at the line, '
      + (LESS
          ? 'writes more than one sentence on a summary line, puts bodies on most symbols or any body '
            + 'past a couple of sentences (a tiny body in a few spots is fine), or pads an Args / '
            + 'Returns / Raises entry into prose (short single sentence entries are REQUIRED where '
            + 'they apply)'
          : 'writes any docstring past two sentences, any body paragraph, or any Args / Returns / '
            + 'Raises section')
      ...
      + 'chains spliced with colons, `->` arrows) instead of short sentences in the voice, '
      + 'or docstrings a symbol with nothing to say. Among the compliant files '
      + 'prefer the one that says the necessary thing in the fewest, best words: a pass that crammed a '
      + 'fact into a compressed clause loses to one that dropped a minor fact and reads clean. '
    ...
    job = 'YOUR PRIMARY JOB IS HOW THE COMMENTS READ. Pick the file whose comments are the best-WORDED at '
      ...
      + 'same bar covers inline `#` comments (short spoken sentences, never note-fragments or `->` '
      + 'arrows) and each docstring\'s first line: a summary that goes flat while the body keeps the '
      + 'voice loses.\n\n'
