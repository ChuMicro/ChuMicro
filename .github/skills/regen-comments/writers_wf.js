...
function tightPrompt(n) {
  ...
  return voiceBlock
    ...
    + 'register -- never notation or `->` arrows. Drop none. Weave a warning into the sentence that '
...
function voicedPrompt(n) {
  return VOICE_PARA + ' Every docstring and comment is fully in that voice.\n\n' + sampleBlock()
    ...
    + 'above its line, in the same register. Each ledger fact lands exactly once; drop none. Weave a '
    + 'warning into the sentence that states it -- never behind scaffolding like "one thing to know" or '
    + '"worth noting". Length follows '
