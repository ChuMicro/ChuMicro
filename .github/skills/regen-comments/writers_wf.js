...
function discipline() {
  ...
  s += '## Say it once, in its tightest form\n\n'
    + 'You write the comment a reader needs and then you stop. Four habits define you.\n'
    + '- Lead with behavior, at the right altitude. A METHOD or FUNCTION summary says what a caller GETS or '
    + 'what CHANGES, not the internal steps that produce it. "Returns the decoded frame, reading more bytes '
    + 'when the buffer is short" beats "Reads bytes, decodes them, and returns the frame". A CLASS or MODULE '
    + 'summary instead says what the thing IS or is FOR, its role -- do not force it into caller-action '
    + 'phrasing.\n'
    ...
    + '- Weave related facts, do not pile them. A consequence belongs woven into the clause it follows from, '
    + 'joined by a real connective (so, because, which), never bolted on with a comma and never chopped into '
    + 'a string of short fragments. "A retry waits longer each attempt, so a flaky call can stall for '
    + 'seconds" reads better than "A retry waits longer, that wait compounds, and a call can stall".\n\n'
...
function genPrompt(n) {
  return 'You will add docstrings and comments to a Python file. Follow this discipline exactly:\n\n' + DISC + '\n'
    + '## Your voice\n\n' + VOICE_PARA + '\n\n'
    + 'WORK IN TWO STEPS. STEP 1: Read ONLY the code at ' + FILE + '. For each METHOD or FUNCTION decide, from a caller\'s point of view, what they GET or what CHANGES when they use it -- that caller\'s-eye sentence IS its summary. For a CLASS or MODULE, instead say what the thing IS or is FOR, its role -- that often reads better than a forced action verb. Either way do not look at the ledger yet, and never let internal mechanism lead a summary. '
...
function consolPrompt() {
  ...
  return 'You are a PER-SYMBOL consolidation judge (fixture-agnostic — no trap list, no knowledge of this '
    ...
    + 'documented THAT symbol best, judged IN THIS ORDER: (1) CORRECT against the code; (2) carries every '
    + 'NON-DERIVABLE must-carry fact from the ledger that pertains to it -- a fact a reader gets straight '
    + 'from the signature or the code is NOT must-carry and must not pad the prose; (3) SHORTEST that does '
    + 'so: among correct candidates that carry the must-carry facts, prefer the one that says it in the '
    + 'fewest words; a longer candidate wins ONLY if it carries a non-derivable fact the shorter one drops, '
    + 'never for extra wordiness or for restating derivable facts; (4) best written; (5) no phrase lifted '
    + 'verbatim from the ledger. The BODY paragraph earns its place only by carrying a fact the summary '
    + 'cannot -- drop it if it just restates the summary at length. State each fact ONCE, at the single most '
    + 'relevant symbol; do not repeat a fact in a class docstring and its method. Mix freely across '
    + 'candidates.\n'
