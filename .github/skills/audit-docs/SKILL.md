...
**Read for the structural defect, the biggest miss no regex catches.**  A sentence whose subject is an abstraction joined by a weak verb to a coined noun that hides an action (*"its floor is the WFI-idle that `ipoll` gives"*) reads as sludge with no banned word in it.  Rewrite so something concrete acts (*"a connected board idles the CPU between events, which is what `ipoll` does"*).  The test is a read, not a grep: say each sentence the way you'd say it out loud to a colleague, and rewrite any you would not say that way.  This rule is required, not a soft hit.  No lint can flag it, which is exactly why the read has to.  Full rule and worked faults in [`agent-style-guide.md` § Concrete subject, real verb](../../../docs/contributing/agent-style-guide.md#concrete-subject-real-verb-the-structural-rule).
