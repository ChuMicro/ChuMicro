...
`def greet(name): return "hi " + name` plus `caller.py` containing `from pkg.mod import greet`;
commit 2 edits `greet` to `def greet(name, loud=False)` and changes its return. Build it with
plain `git init` / `git add` / `git commit` (any session can fabricate this in four commands).
