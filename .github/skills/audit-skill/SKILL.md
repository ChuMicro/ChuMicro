...
- The speaker reframes and never re-grounds: per item it returns a rewritten title, summary, why, and fix built only from that entry's facts, each card readable alone by someone who has never opened the audited skill, exact replacement text and commands passed through character-for-character. Each chunk of cards is written twice and a best-of-2 selector picks the richer, more legible whole pass — never merging across passes. It also returns the page `intro_html`, the `option_help` legend, and the Step 6 mode-gate wording.
