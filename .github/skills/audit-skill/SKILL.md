...
**Harness-claim verification.** A finding tagged `[harness-claim]` (or any finding you notice resting on a documented-Claude-Code-behavior assertion — a frontmatter field's support status, a cap value, loader or tool semantics) gets verified before it lands in the punch-list: dispatch the `claude-code-guide` agent with the specific claim and require a doc URL in the answer. The personas' rules snapshot the docs and lag the product. Docs confirm the claim → the finding stands, URL in the finding text. Docs contradict it → the audited skill is fine; the stale rule itself becomes the finding, pointed at `.github/skills/new-skill/spec-loader-reader.md` and its mirroring personas as a spec-maintenance fix.
