...
function mergePrompt(lensDump, domainFacts, feedback, prior) {
  return 'You are the MERGER. Consolidate findings from five code-evaluation lenses (correctness, hazard, '
    ...
    + 'guess, not a finding.\n'
    + '- WAIVER GATE: if ' + RUNDIR + '/waivers.json exists and lists waivers (each with a fingerprint = '
    + 'file + symbol + defect, plus the human `note`), a finding whose symbol names the same locus AND '
    ...
    + 'A suppressed finding still gets an id and still renders -- greyed, with the note. Never suppress a '
    + 'finding no waiver matches.\n'
...
const patchPrompt =
  ...
  + '`after` = the replacement text. Keep the patch MINIMAL -- only the lines this finding changes. A '
  + 'structural rewrite confined to ONE contiguous region (a method body, a conditional block) is still '
  + '"replace": `before` = the whole current region verbatim, `after` = the rewritten region. Clarity '
  + 'findings in particular (flatten control flow, rename within a symbol, tighten a block) MUST ship as '
  + '"replace" -- the human decides from the actual rewrite, not from a description of one.\n'
  ...
  + '- kind "manual": ONLY when the change spans multiple disjoint regions of the file (a rename rippling '
