...
* **Long docstrings and comments.** `.py` source ships to devices, so every docstring byte is flash. Multi-paragraph docstrings re-explaining the obvious, or block comments narrating history ("previously this did X"), are flash bytes for nothing. The AGENTS.md "code comments" rule already forbids history-comments; the audit catches docstrings that re-state the signature in prose. Triage with a quick AST walk — `python -c "import ast; tree=ast.parse(open('<file>').read()); print(sum((ast.get_docstring(n, clean=False) or '').count(chr(10))+1 for n in ast.walk(tree) if isinstance(n, (ast.Module, ast.FunctionDef, ast.ClassDef)) and ast.get_docstring(n)))"` — anything over ~30% doc-line density on a small library is a trim candidate. Field reality from the chumicro_config pass (2026-05-11): 50–60% density across 3 modules trimmed to 15–33% saved ~5 KB of `.py` source with no API surface lost.
...
* **Public default-kwarg values that drive `__init__`-time heap allocation.** `MpNvsBackend(capacity=DEFAULT_CAPACITY)` looks like a sizing knob, but if `__init__` does `self._buf = bytearray(self.capacity)`, the default value is the long-lived RAM cost every instance pays. Audit the default against the *documented* use case (boot counters, short tokens, etc.), not against the maximum-supported workload. Field reality: kvstore's MP-NVS default was 16 KB — generous against the ~24 KB partition ceiling, but ~32× larger than the documented "boot counters + timestamps + short tokens" workload needs. The fix is two-part: shrink the default and (if call frequency justifies it, per §4) make the buffer transient too.
* **Asymmetric workloads → asymmetric defaults.** When a library has two paths that allocate independently — RX and TX on a network client, server-side vs client-side on a transport, read-buffer vs write-buffer on a persistence layer — the right default for each side comes from its *own* usage profile, not from a unified "library footprint" target. The two paths can be symmetric in code shape but asymmetric in workload: a subscriber receives broker fan-out + retained-message replay + N concurrent subscribed-topic bursts (RX is noisy → 256 B-class buffer right-sized), while the same client publishes once every N seconds (TX queue stays near zero → 20-slot cap plenty). Field reality: chumicro_mqtt's `max_tx_queue_size` defaulted to 100 packets matching its RX-side `recv_budget_per_tick`-shape generosity; dropped to 20 once the asymmetry framing made the documented use case clear — RX is the noisier side, TX the calmer one, defaults follow workload not code symmetry. When auditing defaults, name each path's documented profile separately before deciding numbers.
...
* **`bytes(view).decode("utf-8")` → `str(view, "utf-8")` for utf-8 decode.** The 3-arg `str()` constructor accepts any bytes-like (including memoryview) and skips the intermediate `bytes` copy that `bytes(view).decode(...)` allocates. Bench-validated -25% allocation per call on MP 1.26 + CP 10.2 unix-port (96 vs 128 bytes per call). Worth more in integration: a `chumicro_msgpack` `unpackb` of a 4-string payload dropped -37% (961 → 609 bytes) because each call did multiple decodes.
* **`buffer.extend(struct.pack(fmt, value))` → pack_into on a pre-extended slice.** The encode-side counterpart to the `unpack_from` pattern below. `struct.pack` allocates a fresh `bytes` object per call that's immediately copied into the destination and freed; `pack_into` writes directly into a pre-extended bytearray slice. Pattern: a small `_append_packed(buffer, fmt, value, zero_bytes)` helper plus module-level `_ZERO2 = b"\x00\x00"` / `_ZERO4 = b"\x00\x00\x00\x00"` literals to skip per-call zero-byte allocation. Bench-validated -50% allocation per pack on MP 1.26 (64 vs 128 bytes per call). For signed single-byte fields (`>b`), `buffer.append(value & 0xff)` skips struct entirely. **Important:** this is an established hot-path pattern *for new code*; existing libraries (`chumicro_mqtt._wire`, `chumicro_requests._wire`, `chumicro_websockets._wire`) still use `extend(struct.pack(...))` and should converge on `_append_packed` in their own embedded-audit passes — flag the divergence rather than rewriting an unaudited library mid-pass.
...
* **Sender-controlled `bytearray(N)` / `bytes(N)` allocations — the heap-DoS vector.** Trace every allocation in inbound-parse code and ask "what's the maximum N this can take?" When N is derived from a field the peer controls (an MQTT remaining-length varlen, an HTTP `Content-Length` header, a WebSocket frame length), and the allocation is unbounded, a hostile peer can request whatever heap you have. Static lint won't catch it — the line `self._foo = bytearray(payload_remaining)` looks ordinary in code review. The fix has two shapes: (a) bound N at the documented cap-knob (`max_message_bytes`, `max_body_bytes`) and refuse / drop above it; (b) reuse a pre-allocated steady-state buffer as a rolling sink and discard the bytes as they drain. Field reality from the chumicro_mqtt audit (2026-05-12): the "oversized message" path allocated `bytearray(payload_to_drain)` sized to the remaining payload — a 1 MB hostile inbound PUBLISH allocated 1 MB on a 256 KB-RAM board. Three prior audit passes had walked past this — flag every sender-controlled `bytearray(N)` even when it sits next to a comment that claims to be the heap-safe degraded path.
...
* **A buffer earns long-lived status by call frequency, not by the word "reusable" in a comment.** A long-lived buffer pays off when it's touched many times per session or in a hot path; a buffer touched 1–2 times per object lifetime is transient masquerading as persistent — allocate it inside the call site and let GC reclaim. Field reality: `MpNvsBackend` held `self._read_buffer = bytearray(self.capacity)` for the object's lifetime to "reuse across loads," but `load()` runs at construction + on explicit `reload()` only (never on the commit hot path), so the buffer pinned 16 KB on a 256 KB-RAM ESP32 for zero per-call benefit. Moving the allocation inside `load()` made the buffer transient and dropped the long-lived RAM cost to near-zero. When the audit sees a `_buf = bytearray(N)` in `__init__`, count actual touches per session before defending the long-lived shape.
* **Transient per-call buffers** — encoder output for `packb`-shaped functions, formatter accumulators, anything built once per call and immediately returned — DO NOT default to pre-allocate-and-position-track. The obvious "`bytearray(64)` + write cursor + `bytes(memoryview(buf)[:pos])` return" pattern measured **+33% to +135% allocation** across realistic payload sizes (47–500 B) vs plain `bytearray()` + `.append()` / `.extend()` + `bytes(buf)` on MP 1.26 unix-port (msgpack audit, 2026-05-11). MicroPython's bytearray uses capacity-doubling internally; `buf.append(byte)` is one C-level call, while `buf[pos] = byte; pos += 1` is two Python operations + an attribute lookup, and `bytes(memoryview(buf)[:pos])` adds a wrapper allocation on top of the bytes copy. Don't pre-allocate transient buffers without bench-measuring against the capacity-doubling baseline first.
...
* **Lazy-import fragmentation hypothesis.** When evaluating a proposal to defer a heavy module's import (e.g. moving `from chumicro_msgpack import unpackb` inside a function), ask the textbook fragmentation question: between the deferred-from point and the deferred-to point, what else allocates? Short-lived churn in that window leaves small free regions; the deferred module's long-lived globals land in those regions rather than contiguously, and later hot-path allocations have to fit in the leftover fragments. **Usually moot:** module-top imports load long-lived state, the deferred-to point is right after (top of `main()`, before any networking or buffer churn), so the heap layout is essentially identical to eager. **Sometimes real:** if the deferred-to point is mid-runtime (after wifi-connect, after MQTT-subscribe, after handling traffic), then short-lived churn has fragmented free space by then and the deferred import lands in holes. The audit should call out when the deferred-to point is mid-runtime vs boot-time — only the former carries the fragmentation risk, and only the former justifies the simplicity cost of a function-scope `# noqa: PLC0415` import.
...
* **`__slots__` reality — remove from cross-runtime device libraries.**
  * **Bench-validated** on MicroPython 1.26 unix port and CircuitPython 10.2.0 unix port (timing-audit pass, 2026-05-11): 100 instances of a slotted class consume the same heap as 100 instances of an unslotted class (10688 bytes on both runtimes), and `instance.unknown_attribute = 1` is accepted without raising `AttributeError`. The MP/CP parser accepts the `__slots__` syntax for compatibility but doesn't use it for instance layout or attribute enforcement.
  * The on-device picture: zero RAM saving, zero attribute locking, plus a worse outcome — CPython's real `__slots__` would raise `AttributeError` on a typo'd attribute write, but MP/CP silently accept it. The same test sweep reports green on one runtime and is blind to the failure on the other two.
  ...
  * Reproduce locally: `.tools/micropython-v1.26.0/ports/unix/build-standard/micropython -c '<test>'` and the equivalent under `.tools/circuitpython-*/ports/unix/build-standard/`. Both runtimes give the same answer.
...
* **Every code example in docs:** does it import successfully against the current code? Can it run on at least CPython without modification? Field reality: kvstore's getting-started snippet called `ticks_ms()` without showing `from chumicro_timing import ticks_ms` — a cold reader copy-pasting would hit `NameError` before the example demonstrated anything.
* **Doc bugs cluster — find one, audit every numeric and symbolic claim in the same file.** When one quantitative or symbolic claim is wrong, re-read every other number and symbol in the file before moving on. kvstore's guide.md surfaced three drifts on a single read: a missing import (above), a SAMD21↔SAMD51 chip-family swap, and a capacity number that disagreed with both the code default and the ADR. Doc-writers introduce errors in batches; auditors should find them in batches.
...
* **Don't trust intuition alone for buffer-strategy refactors.** The msgpack audit (2026-05-11) found that the obvious "pre-allocate to skip extend churn" intuition measured *worse* than the current capacity-doubling baseline at every realistic payload size. Bench BEFORE proposing the refactor; the bench can flip the recommendation.
* **Distinguish "eliminates cost" from "defers cost."** A measured saving that defers an allocation (lazy import, on-demand decode, late-bound callback) is only a real saving if the deferred-to point is far enough from the deferred-from point that the program enters a different state in between — different working set, different live longevity, different fragmentation context. When the deferred-to point is "a few stack frames later with no other work in between," peak RAM is unchanged and the optimization is timing-only. Always pair the micro-bench number with a real-usage timing trace: when does the cost actually get paid in the typical app shape? Field reality from the chumicro_config pass (2026-05-11): lazy `from chumicro_msgpack import unpackb` measured a clean 7.1 KB import-time saving on MP 1.26 and CP 10.2 unix-port — but real apps call `load_runtime_config()` at the top of `main()`, a few frames after module-top imports complete, so the deferred 7.1 KB lands almost immediately and the import-time saving doesn't manifest in practice. Recommendation flipped from "MEDIUM apply" to "TASTE-CALL skip" once the timing was accounted for.
...
* **When a network call fails inconsistently across contexts, instrument the function — don't theorise.** A connect that succeeds from the REPL but fails from `main.py` is tempting to explain as a wifi-settle / lwIP-TIME_WAIT / cyw43-power-save race. None of those are usually it. The fastest path is to add `print()` statements around the suspect function (`socket.getaddrinfo`, `sock.connect`, the factory closure) printing the **arguments it's actually receiving** and run again. The diagnosis you can't theorise into is usually "the arguments aren't what you think they are." Field reality from the chumicro_mqtt bench session (2026-05-12): an hour spent theorising wifi-settle timing for an `ECONNABORTED`; four `print()` lines added to `connect_tcp` on-device immediately showed `host=test.mosquitto.org` (from `secrets.toml`) instead of the expected `host=172.16.1.15`, and the broker-side mosquitto log confirmed the connections-and-immediately-closes pattern. Static review and theorising can't substitute for "what value is this function actually called with right now." Tag this as `cpu` or `runtime` depending on which dimension the fix lands in.
...
**Prior art outside `.tools/` — when the library has a predecessor, do a side-by-side compare.** If the user references "we had an MQTT client that worked before — see `<path>`," or if the library's docstrings credit an upstream implementation, read that reference top-to-bottom under the same audit lens. Compare it against the current library along three axes: (a) **missing concepts** — what tiers / modes / lifecycle phases does the reference distinguish that the current library has collapsed? (b) **buffer sizing** — what fixed-size buffers does the reference reserve where the current library allocates dynamically, and vice versa? (c) **error pathways** — does the reference raise / disconnect / fault on inputs the current library silently accepts? Field reality from the chumicro_mqtt audit (2026-05-12): the user pointed at `~/circuitpython/pythonProject3/basefilesystem/lib/basefs/mqtt_client.py` — a previous-generation implementation. A 30-minute side-by-side surfaced the missing intact-delivery tier (the current library treated every PUBLISH > rx_buffer_size as oversized; the reference distinguished "fits in heap, deliver intact" from "way too big, discard via rolling sink"), the missing fixed `DEGRADED_BUFFER_SIZE` constant, and the missing SUBACK 0x80-rejection auto-fault — none of which had been flagged in three prior audit passes. Reference comparison is the single highest-yield move when prior art exists. Cite the reference path in punch-list findings so the user can verify the comparison.
