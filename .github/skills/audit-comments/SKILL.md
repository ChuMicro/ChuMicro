...
* **`the`-density as a symptom (not a target).**  Stacked definite articles and sentence-initial `the` before brand names usually mark prose written by accretion, not by a writer.  Two sources of truth, both cited not re-derived here: `/audit-docs` dim 2 for tic detection in existing prose, and AGENTS.md → Writing tone "`the X` only when X is a specific established instance" for the per-noun forward-reference test on REWRITE drafts — inherited `the`s the rewrite did not earn compound across passes the same way the superlative tic does, so the test goes on the draft, not only on the original.  Reduce as a *symptom* of low signal.
