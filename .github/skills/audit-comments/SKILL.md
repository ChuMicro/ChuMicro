...
* **REWRITE** — degraded: illegible, buried, label-only, or so trimmed it no longer says what the thing is.  **Discard it.  Read the code.  Write a new comment from scratch.**  This is the default action for the residue described above — do not try to salvage word-by-word.
...
* **`the`-density as a symptom (not a target).**  Stacked definite articles and sentence-initial `the` before brand names usually mark prose written by accretion, not by a writer.  Use the `/audit-docs` dim-2 grammar-tic guidance (that skill is the source of truth — cite it, don't re-derive the list here).  Reduce as a *symptom* of low signal; do not strip every `the` — *"the LED"*, *"the loop"*, *"the request"* are specific singular nouns and dropping the article reads wrong (`/audit-docs` "What NOT to do" applies verbatim).
* **AI-tic vocabulary in comments/docstrings** — run the `/audit-docs` dim-2 standing regex over `src/` comments, not just markdown.  Same handling (drop / replace / case-by-case).  Do not carry a private copy of the list — drift per `/audit-skill` dim 6; `/audit-docs` dim 2 + `feedback_doc_writing_taste` are the source of truth.
...
* **Dated incident / bench log** — *"2026-05-09 ESP32-S2 bake"*, *"empirically the slowest we've observed"*.  The dated subset is `CHU012`'s; the dateless verb-anchored subset (*"X now that Y landed"*) is judgment, owned here.  DELETE — per-change rationale lives in the commit message.
...
3. **Run the dimension sweeps** (dims 1–5).  Greps that have a source-of-truth elsewhere (AI-tic regex → `/audit-docs` dim 2; essay-bloat ratio script → `/audit-library` field-reality) are *invoked by reference* — run them, don't transcribe them into this skill.
4. **Draft replacement text for every REWRITE finding.**  A REWRITE finding without proposed new prose is not actionable — write the new comment as part of the punch-list so the user reviews the actual replacement, not a promise.
...
* **Don't transcribe sibling source-of-truth lists.**  The AI-tic regex (`/audit-docs` dim 2), the essay-bloat ratio script (`/audit-library` field-reality), the dated-history lint subset (`CHU012`/`CHU006`) are owned elsewhere — invoke by reference; a private copy here is drift (`/audit-skill` dim 6).
