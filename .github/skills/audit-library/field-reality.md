...
CHU006 catches `.scratch/` in publishable `src/` directly — the broader rule-set behind that lives in [Decision 0079](../../../plans/decisions/0079-prose-drift-mechanization.md) (workstream archived at [`plans/workstreams/archive/chu-prose-isolation-lint-gaps.md`](../../../plans/workstreams/archive/chu-prose-isolation-lint-gaps.md) for deeper rationale).  The `the (one|single|sole) X` definition tic and the `the (next|first|only|new|sole) X` forward-reference tic stay *not* mechanizable per the same ADR's close-out: bench against publishable src returned 35 candidate hits, zero genuine tics, and "X is the one Y that Z" really is the canonical shape of Decision 0077-class invariants, so any mechanization is either noqa-heavy noise (which Decision 0074 rejects) or misses the real tic surface.  Don't re-derive these by hand each audit; reconcile against Decision 0079 and don't let this skill drift from it.
