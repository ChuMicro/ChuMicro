...
* **Walkers belong with what they walk, not with what they read.**  When extracting modules, helpers that iterate a collection and `isinstance(item, <Type>)` against its members belong in the module that defines `<Type>`, not in the module that owns the collection's source.  The natural-sounding instinct "this helper reads `session.items`, so it goes in the session-helper module" is wrong when the helper also needs to know the *type* of items — it induces a cycle that lazy imports paper over.  The walker module wants the data *and* the type together; the type's home is the walker's home.  Worked case: the `_device_closure_source_dirs` / `_device_is_unit_sweep` / `_device_own_source_dirs` family of `chumicro_pytest_device` belong with `DeviceTestItem` in `collection.py`, even though they read `session.items` — the original draft put them in `session.py` and immediately needed a `from .collection import DeviceTestItem` lazy import.

* **An induced cycle during extraction is a finding, not just a problem to paper over.**  If the proposed split produces an import cycle, the cycle is diagnostic information: the original module bundled two responsibilities whose true boundary sits differently from where the cut was sketched.  Re-draw the boundary around the cycle (or move the smaller responsibility across the cut) — almost always a linear graph exists.  Only reach for a lazy import after confirming the structural fix won't work, and write the lazy import's rationale into the comment so a later auditor doesn't undo the analysis.  Complements "Break import cycles with a leaf types module": that rule names the *fix*, this one names the *signal*.
