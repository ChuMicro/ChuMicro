...
| Descriptive names, no abbreviations | No single-letter variables (except `_`). Spell out `error`, `file`, `index` — not `e`, `f`, `i`. A short [allowed abbreviations list](plans/decisions/0022-naming-conventions.md) covers `dir`, `args`, `cmd`, `env`, `err`, and a few more ([Decision 0022](plans/decisions/0022-naming-conventions.md)) |
