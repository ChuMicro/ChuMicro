...
Strategy C of setup-schema-reconciliation: when the upstream starter
gains a new key the user doesn't have, append it in place — comments
preserved, existing values untouched.
"""
...
class TestPrivateHelpers:
    ...
    branches (path missing from starter, intermediate-table creation)
    can't be reached via the public driver because
    ``collect_missing_starter_paths`` only returns paths known to
    exist in the starter, so test them in isolation."""

    def test_toml_writer_skips_paths_missing_from_starter(
        self, tmp_path: Path,
    ) -> None:
        from chumicro_workspace.additive_apply import _append_missing_toml  # noqa: PLC0415

        # Path "nope.gone" doesn't exist in starter — writer skips it.
    ...
    def test_yaml_writer_skips_paths_missing_from_starter(self) -> None:
