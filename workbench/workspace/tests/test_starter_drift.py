...
fields the canonical workbench-owned starter has gained since the
user materialised their ``workspace.yml`` / ``secrets.toml``.
No auto-application; pure print.

Tests inject synthetic starters by monkeypatching the canonical
reader so each case can exercise a tailored diff scenario.
"""
...
def _patch_workspace_yml_starter(
    ...
    """Make :func:`read_workspace_yml_starter` return *content* for the test."""
    monkeypatch.setattr(
...
def _patch_secrets_toml_starter(
    ...
    """Make :func:`read_secrets_toml_starter` return *content* for the test."""
    monkeypatch.setattr(
...
class TestCollectMissingStarterPaths:
    ...
    def test_user_matches_starter_returns_empty(
    ...
    def test_empty_starter_returns_empty(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        ...
        _patch_workspace_yml_starter(monkeypatch, "# comment-only\n")
    ...
    def test_uses_canonical_workbench_starter_when_unpatched(
        self, tmp_path: Path,
    ) -> None:
...
class TestPrintStarterDriftReport:
    ...
    def test_no_drift_prints_nothing(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        starter = "defaults:\n  wifi:\n    ssid: ap\n"
        _patch_workspace_yml_starter(monkeypatch, starter)
    ...
    def test_drift_lists_each_path_and_names_source(
        ...
        # Source label points at the workbench-owned canonical starter.
        assert "workbench-owned starter" in output

    def test_pluralisation_matches_count(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        starter = (
            "defaults:\n  wifi:\n    ssid: ap\n"
...
class TestSecretsTomlDrift:
    ...
    def test_workbench_starter_ships_real_placeholder_keys(self) -> None:
        ...
        # (Strategy C) can append new starter keys to a user file
        # without round-trip data loss.
