...
(ruamel attaches comments to keys; a header with no following key has
nothing to hang on).  So each managed key is instead found and
...
comment-aware round-tripper, and is acceptable for keys the user is
told are machine-owned.
"""
...
def _block_regex(key: str, marker: str) -> re.Pattern[str]:
    ...
    starts at the marker line; with a bare ``key:`` (legacy / hand-
    written) it starts at the key line.  Either way the whole block
...
def sync_managed_block(
    ...
    *child_lines* are the already-indented YAML lines that go under
    ...
    if match is not None:
        ...
        # No existing block — append.  Add a blank-line separator
        # from prior content, but leave an empty file producing the
        # bare block alone.
