...
class TestResolveEffectiveDeployMode:
    ...
    def test_pi_pico_w_cp_mqtt_auto_upgrades_to_flash(self) -> None:
        """RAM-mode chumicro-mqtt on Pi Pico W CP auto-upgrades to flash.

        Without library-aware routing the test would inherit the
        ``defaults.deploy_mode: ram`` and OOM on the inline-bootstrap
        AST build (the parser needs ~14 KB of heap on top of the
        chunk's own size).
        """
        device = DeviceEntry(
            identifier="pi-pico-w-circuitpython-board",
            runtime="circuitpython",
            address="/dev/cu.x",
            deploy_mode="ram",
        )
        # Without library_name, the natural mode is preserved.
        assert device_testing.resolve_effective_deploy_mode(
            device, None,
        ) == "ram"
        # With library_name="mqtt", the constraint kicks in.
        assert device_testing.resolve_effective_deploy_mode(
            device, None, library_name="mqtt",
        ) == "flash"

    def test_explicit_ram_override_wins_over_constraint(self) -> None:
        """``--chumicro-deploy-mode ram`` reproduces the OOM intentionally.

        The override is the user's escape hatch — they may want to
        confirm the constraint is still firing on their hardware
        before flipping it to flash.
        """
        device = DeviceEntry(
            identifier="pi-pico-w-circuitpython-board",
            runtime="circuitpython",
        ...
        assert device_testing.resolve_effective_deploy_mode(
            device, "ram", library_name="mqtt",
        ) == "ram"

    def test_unrelated_library_unaffected(self) -> None:
        """Only chumicro-mqtt has an entry for Pi Pico W CP today."""
        device = DeviceEntry(
            identifier="pi-pico-w-circuitpython-board",
            runtime="circuitpython",
            address="/dev/cu.x",
        ...
        assert device_testing.resolve_effective_deploy_mode(
            device, None, library_name="timing",
        ) == "ram"

    def test_unrelated_board_unaffected_by_mqtt_constraint(self) -> None:
        """The Pi-Pico-W constraint doesn't fire on other CP boards."""
        device = DeviceEntry(
            identifier="lolin-s2-circuitpython-board",
            runtime="circuitpython",
            address="/dev/cu.x",
            deploy_mode="ram",
        )
        # Lolin S2 has 2 MB SRAM (effectively, with PSRAM) — no OOM.
        assert device_testing.resolve_effective_deploy_mode(
            device, None, library_name="mqtt",
        ) == "ram"

    def test_machine_field_in_extra_matches_fingerprint(self) -> None:
        """A renamed device with ``hardware.machine`` set still matches."""
        device = DeviceEntry(
            identifier="back-porch-cp",  # user-renamed identifier
            runtime="circuitpython",
            address="/dev/cu.x",
            deploy_mode="ram",
            extra={"hardware": {"machine": "Raspberry Pi Pico W"}},
        )
        assert device_testing.ram_mode_too_constrained(
            "mqtt", device,
        ) is True
        assert device_testing.resolve_effective_deploy_mode(
            device, None, library_name="mqtt",
    ...
    def test_no_library_name_skips_constraint(self) -> None:
        """``library_name=None`` opts out of the constraint check."""
        device = DeviceEntry(
            identifier="pi-pico-w-circuitpython-board",
            runtime="circuitpython",
            address="/dev/cu.x",
